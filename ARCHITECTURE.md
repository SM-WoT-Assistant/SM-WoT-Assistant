...
- `config.load_version()` (`config.py`) reads VERSION and appends `" Beta"` unconditionally — all window titles display `"1.0.70 Beta"`; `build.py` controls the GitHub release title and RTDB `display_version` field with `is_beta=True` unconditionally but does not affect runtime `load_version()`
