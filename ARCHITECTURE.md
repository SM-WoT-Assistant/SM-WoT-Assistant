...
- Location: `stats_ai.py`, `ai_engine.py`, `stats_data.py`, `service_messages.py`, `generate_prompt_v2.py`, `admin_build_generator.py`, `_fill_all_builds.py`, `parse_tiers_devices.py`, `admin_auth.py`
- Contains: StatsAI class, Firebase sync methods (`_sync_popular_tanks()`, `_sync_builds()`), build validation (`_is_build_complete()`), cache management (version-tracked `ai_builds_cache.json` with version + scripts_fingerprint, shared `_is_cache_expired` for 30-day expiry), bulk-cache save via `_save_ai_build_cache_bulk()`, deduplicated fallback reporting via `firebase_reporter.report_fallback()`; equipment/consumable/skill mappings generated at runtime from `.mo` files by `stats_data.py:generate_mo_maps()`; admin daemon `admin_build_generator.py --listen` polls `pending_updates/builds` and regenerates all builds via Gemini, using `_fill_all_builds.py:parse_build` to parse responses and `generate_prompt_v2.py:generate_prompt` for prompt construction; a 24h daily fill sweep (strict completeness via `strict_build_incomplete()` — every `equipment_1`/`equipment_2`/`consumables_1`/`consumables_2` loadout non-empty and every client crew role present with non-empty perks, roles normalized via `_normalize_crew_role`; `scan_incomplete_builds()` fetches `builds/tanks` and returns `{tag: [missing]}`; `_run_daily_sweep()` queues them via `pending_updates/builds`) — skipped while a daemon or GUI generation is in progress (mutual exclusion: GUI checks pending status, daemon checks `admin_app/status` + `last_seen` freshness); Chrome sessions always drive an isolated profile copy in `%TEMP%\sm_wot_admin_chrome_profile` (Chrome 136+ blocks remote debugging on the default user-data-dir) positioned fully off-screen (`--window-position=-32000,-32000` — no visible window above other apps), with `_kill_chrome_matching()` targeted cleanup, `_DRIVER_RETRY_MARKERS`-based 3-attempt retry in `_create_driver()`, and one mid-run reconnect per tank in `generate_builds()`; `generate_builds` returns `(ok, done_tags, reasons)` — reasons is `{tag: "category: detail"}` for every failed tag plus a `summary`; every failure category surfaces through 5 channels (console print, GUI log + tray balloon, RTDB `pending_updates/builds` error message, and `report_fallback` to `error_reports` — admin.html Errors); `unknown_tank`/`client_parse_fail`/`no_prompt`/`ai_no_response`/`ai_parse_fail`/`upload_fail` are explicit and never collapse into a bare `False`; the consecutive-failure registry (`_failures` node in `.tank_extract_manifest.json`, `update_manifest_failures`/`exclude_failed_tags`, threshold 3) excludes a repeatedly-failing tank from hourly detection and the daily sweep until its scripts.pkg fingerprint changes again (new game version grants a fresh attempt; success resets the counter via `update_manifest_for_tags`) — no silent infinite FAILED loops; `detect_changed_tanks` reports a missing `scripts.pkg` via `report_fallback` instead of silently returning `[]`; `_fill_progress.json` (crash-resume for force/batch paths) lives in `config.USER_DATA_DIR`, and a stale index is guarded (`start_idx > len(all_tags)` falls back to 0); `save_prompt` caches locally only after a successful `_upload_prompt`; `_update_builds_version()` bumps `builds/version` only when `ok_count > 0` (failed runs do not force client re-syncs); RTDB writes publish through `admin_auth._rtdb_url_with_token()` (ID token — skipped with a printed warning when `admin_creds.json` is missing); the change-tracking manifest (`update_manifest_for_tags`) advances only for tags actually uploaded to RTDB, and queue tags missing from the stale `tank_db.json` are built up from client data (`_tank_record_from_client` from list.xml — raises with a `list.xml parse failed` reason instead of returning a silent `None` — + `_slots_and_crew_from_client` from the vehicle XML, persisted via `_persist_client_tank_data` into tank_db.json / tank_slots_full.json / crew_builds.json + generate_prompt_v2 module-level dicts) — new client tanks are never silently dropped (F141_Durendal fix, 07.08.2026)
...
- Location: `firebase_identity.py`, `firebase_reporter.py`, `firebase_drawings.py`, `firebase_groups.py`, `admin_auth.py`, `public/` (Firebase Hosting static website), `firebase.json`, `.firebaserc`, `database.rules.json`
- Contains: Identity system (`register`, `login`, `connect`, `disconnect`, `check_nickname_available`, `change_nickname`, `change_pin`, `is_registered`, `is_connected`, `get_nickname`, `get_user_id` — nickname + PIN stored as SHA-256 hash in `identity.json` under `config.USER_DATA_DIR`, synced to RTDB `users/{user_id}`), RTDB REST API client (PUT/POST/GET with Firebase API key auth and `_rtdb_url()` path builder), error reporting (`report_error(type, stage, error_text, blocked=False, details=None)` with structured error types and `blocked=True` for fatal startup errors that show `_show_verification_error()` and exit; `report_fallback(source, context, error_text, level)` — lightweight wrapper for non-fatal data fallback events, deduplicated per caller+context via `_REPORTED_FALLBACKS` set), version ping/check via `versions/` node with `_pick_latest()` sorting by release_date then semver, drawing publish/load/delete via `schemes/` node, auto-update dialog in `main.py` and `launcher.py`, group system (`create_group`, `join_group`, `leave_group`, `get_user_groups`, `publish_to_group`, `update_group_scheme`, `get_group_schemes`, `get_group_schemes_meta`, `import_between_groups`, `delete_group_scheme` via PUT null, `invalidate_group_schemes_cache`) with RTDB `groups/{gid}/` and `user_groups/{uid}/` nodes; admin write auth (`admin_auth.py:get_id_token()` — Firebase ID token via `accounts:signInWithPassword` from `admin_creds.json` under `config.USER_DATA_DIR`, cached with refresh-token auto-refresh; `_rtdb_url_with_token()` strips any existing `?auth=` param because RTDB uses the first auth parameter) used by `admin_build_generator._put_json` and `build.py:write_version_to_rtdb` — without credentials these writes fail visibly (skipped with a printed warning); secured `database.rules.json` (root deny; `versions/`, `builds/`, `prompts/`, `popular_tanks/` write only for the admin uid; `installations/`, `service_events/`, `admin_app/` auth-gated reads; `error_reports/` append-only via wildcard; `drawings/` read-only)
...
1. `log_reader.py:LogWatcher._run` tails `python.log` with regex patterns; `hangar_re` matches client hangars and event spaces (`hangar\w*` and `h<N>_*` — e.g. `h42_Wot_Bday_2026`), and both are excluded from arena detection (event hangars are not battles); type-to-mode mapping: 1→ctf (Standard), 2→domination (Encounter), 3→assault (Storm), 4→comp7 (Onslaught), 5→comp7_light (OnslaughtLight)
2. On map detection → `main.py:on_battle_detected` → auto-sync map selector and battle mode filters; when unhidden from tray (`unhide_on_battle`), restores via `_battle_restore_from_tray()` through `root.after(0, ...)` (no Tk from the watcher thread) and always switches to the edit window (`toggle_editor` if mode != edit) so the battle starts with the editing view; sync via `safe_battle_sync()` skips the `switch_to_maps(2)` full rebuild when already on the maps view with mode 2
...
5. On battle end → `main.py:on_battle_ended` → if the battle restored the app from tray (`_restored_by_battle` flag), minimize back to tray; otherwise optionally return to editor with last battle's map
...
**Auto-Update Flow:**
1. **[Launcher path]** `launcher.py` is the preferred update path — auto-detects the installed version via `_detect_installed_version()` (regex on EXE filenames, handles "Alpha" suffix). On startup `_clean_old_versions()` removes stale EXEs. Then checks RTDB synchronously (`check_for_updates_sync()`) before launching the main app. If a newer version is found, it downloads the setup to `%TEMP%` with a progress bar on its splash, runs the NSIS installer silently (`/S /NCRC`), cleans up old versioned EXEs, then spawns `SM WoT Assistant Launcher.exe --skip-splash` (which relaunches the main EXE via `_find_main_exe()`) and exits. After install, `installer.nsi` auto-launches the new version via `Exec`
...
5. Latest version is determined via `_pick_latest()` which selects the version with the highest `release_date` from the RTDB `versions/` node, using semver as tiebreaker; the `versions/latest` pointer (with `release_date: "9999-12-31"`) always points to the canonical latest; `compare_versions()` compares semver tuples and strips " Alpha" suffix before comparison; `display_version` field holds the human-facing label (e.g. `"1.0.40 Alpha"`)
6. On build → `build.py:write_version_to_rtdb()` publishes version metadata (version, release_date, download_url, build_size) to RTDB `versions/{version}/` and updates the `versions/latest` pointer; RTDB publish only happens after a successful GitHub release and audit, and requires an admin ID token (`admin_auth.get_id_token()` from `admin_creds.json`) — without credentials the publish fails with a printed error
...
- Responsibilities: Pre-flight validation (semver, Python 3.12+PyInstaller, NSIS, `gh` CLI, critical source files, git status — dirty working tree blocks build), clean dist/build, PyInstaller onedir via `wot_assistant.spec` + manual `copy_data_files()` (workaround for PyInstaller 6.x DATA TOC bug) + `generate_verify_json()` writes `public/verify.json` and `popular_tanks_seed.json`, `build_tray_watcher()` (onefile) + `_build_launcher()` (onefile), NSIS installer via `run_nsis()`, rename onedir to versioned directory + versioned EXE, build verification (`verify_launcher_bundle()` checks launcher for Tcl/Tk DLLs + `_imaging`), build manifest; then 4-phase release audit: `verify_release_artifacts()` → `create_github_release()` → `audit_github_release()` (deletes release on mismatch) → `write_version_to_rtdb()` → `audit_rtdb_entry()`. All builds are Alpha/prerelease by default (`is_alpha=True` unconditionally, `--prerelease` always set); every build is a GitHub release. `write_version_to_rtdb()` publishes with an admin ID token via `admin_auth.get_id_token()` — without `admin_creds.json` in `%APPDATA%/SM WoT Assistant/` the RTDB publish fails with a printed error and the version is not recorded
...
- Responsibilities: tkinter GUI for build generation, scripts.pkg scanning, WG API game version monitoring, auto-detect changed tanks, generate builds via Gemini, tray icon with balloon notifications, settings persistence; starts minimized to the system tray by default (`start_minimized: True` — new installs; X button minimizes to tray, full exit via Settings, autostart uses `--tray`); daily build fill sweep (`_run_build_fill_sweep()` regenerates strictly incomplete builds directly in a thread — no `pending_updates` trigger, so a concurrently running `--listen` daemon does not double-generate; guards on `self._generating` + pending status; mutual exclusion with the daemon via `admin_app/status`; queue filtered through `exclude_failed_tags` against the manifest `_failures` registry); single-instance mutex `SM_WoT_Assistant_Admin_SingleInstance` restores the existing window on second launch; `_read_admin_version()` reads `admin_version.txt` (shown in window title + Admin Version card); `_log()` appends to `admin.log` in `%APPDATA%\SM WoT Assistant` and reverses the UI log (capped at 1000 widget lines); i18n EN/UK with `_TR_EN` dict and cached UK translations in `admin_uk_cache.json` (seeded from `admin_uk_seed.json`); `_resolve_wot_path()` auto-detects WoT path (CLI `--wot-path` → `admin_settings.json` → main `settings.json` wot_path → common paths, validated via `version.xml`)
...
**Version System:**
- `VERSION` file at project root contains a plaintext semver string (e.g., `1.0.70`)
...
- `config.load_version()` (`config.py`) reads VERSION and appends `" Alpha"` unconditionally — all window titles display `"1.0.70 Alpha"`; `build.py` controls the GitHub release title and RTDB `display_version` field with `is_alpha=True` unconditionally but does not affect runtime `load_version()`

**Build & Packaging:**
- `build.py` — 8-phase pipeline: (1) Pre-flight validation (semver, Python 3.12+PyInstaller, NSIS, gh CLI, critical source files, git status — dirty working tree blocks build), (2) Clean dist/build, (3) PyInstaller onedir + manual `copy_data_files()` (workaround for PyInstaller 6.x DATA TOC bug) + `generate_verify_json()` writes `public/verify.json` and `popular_tanks_seed.json`, (4) `build_tray_watcher()` (onefile) + `_build_launcher()` (onefile), (5) NSIS installer via makensis, (6) Rename onedir to versioned directory + versioned EXE, (7) Build verification (`verify_launcher_bundle()` — launcher Tcl/Tk DLLs + `_imaging`), (8) Build manifest. After all phases a 4-phase release audit runs: `verify_release_artifacts()` → `create_github_release()` → `audit_github_release()` (deletes release on mismatch) → `write_version_to_rtdb()` → `audit_rtdb_entry()`. `write_version_to_rtdb()` and `admin_build_generator._put_json()` publish via admin ID token (`admin_auth._rtdb_url_with_token()`) — RTDB writes fail visibly without `admin_creds.json` (`%APPDATA%/SM WoT Assistant/`, gitignored)
...
- All builds are Alpha/prerelease by default: `is_alpha = True` unconditionally, `--prerelease` always set; " Alpha" suffix added to GitHub release title and RTDB `display_version` field, but filenames remain clean; installer and portable ZIP get `_Alpha` suffix
...
- `installer.nsi` — NSIS script: `InstallDir "$LOCALAPPDATA\SM WoT Assistant"`, `RequestExecutionLevel user` (no admin), kills running tray watcher via `nsExec::ExecToStack 'taskkill /f /im "SM WoT Assistant Tray Watcher.exe"'` (hidden console launch — `ExecWait` would flash a terminal window during install), deletes old `SM WoT Assistant v*.exe`, cleans old `v*.lnk` shortcuts, writes `HKCU\Software\SM WoT Assistant installed=1`, auto-launches new version post-install via `Exec`; shortcuts are version-agnostic
