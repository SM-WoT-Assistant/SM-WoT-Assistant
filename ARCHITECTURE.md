...
- `config.load_version()` (`config.py`) reads VERSION and appends `" Alpha"` unconditionally — all window titles display `"1.0.68 Alpha"`; `build.py` controls the GitHub release title and RTDB `display_version` field with `is_alpha=True` unconditionally but does not affect runtime `load_version()`
