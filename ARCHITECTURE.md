...
- Location: `log_reader.py` (python.log tailing), `window_manager.py` (WinAPI window manipulation), `tray_icon.py` (system tray icon via `Shell_NotifyIconW`), `map_extractor.py` (client .pkg extraction), `tank_extractor.py` (client XML reading), `build_crew_builds.py`, `extract_equipment_loadouts.py`, `build.py` (PyInstaller packaging, always creates GitHub release — no `--release` flag, `gh` CLI required; supports `--beta` flag for "Beta" display suffix while keeping filenames clean semver), `launcher.py` (standalone update-checker + background tray watcher built as `--onefile`, uses `import unicodedata` for PyInstaller compatibility; supports `--tray` watcher mode with WinAPI tray icon and WoT process detection via `CreateToolhelp32Snapshot`; `_clean_old_versions()` removes stale EXEs at startup; Beta-aware EXE detection via regex), `tray_watcher.py` (minimal background process monitor with `CreateToolhelp32Snapshot`, pure stdlib + ctypes, no tkinter/PIL/requests — watches for WorldOfTanks.exe at 5s intervals, launches launcher on game start, closes main app on game stop when `close_with_game` setting enabled), `wot_assistant.spec` (PyInstaller spec for main app)
...
**Startup Flow:**
1. **[Launcher path]** When distributed, `launcher.py` runs first as a standalone `--onefile` executable — shows its own splash, checks RTDB for updates synchronously, downloads and installs the latest version if found, then launches the main app with `--splash-geometry=` to preserve splash position. For Windows autostart, `tray_watcher.exe` runs as a background process (0 windows, pure stdlib + ctypes) monitoring `WorldOfTanks.exe` at 5s intervals via `CreateToolhelp32Snapshot` — launches launcher on game start, closes main app on game stop when `close_with_game` setting enabled. Falls back to `launcher.exe --tray` if tray_watcher not found
...
3. **Splash-screen path:** During splash display, `_start_startup_checks()` calls `firebase_reporter.check_for_updates_sync()` synchronously. If a newer version is found, `_show_splash_update()` renders "New version vX.Y.Z available!" with "UPDATE NOW" / "LATER" buttons directly on the splash canvas. Clicking "UPDATE NOW" triggers `_splash_download_thread()` which downloads the setup to `%TEMP%`, runs NSIS installer silently via `subprocess.Popen` with `/S /NCRC`, waits 0.5 seconds, then calls `os._exit(0)` — no blocking wait, no post-install EXE check, no mutex release. The installer's `Exec` directive auto-launches the new version. The splash remains visible during download with animated dots (`_splash_animate_dots`, 500ms interval). Clicking "LATER" dismisses splash and continues to main UI
4. **Main-UI path:** If no update was found during splash (or splash is disabled), `_check_for_app_updates()` calls `firebase_reporter.check_for_updates()` asynchronously after the main UI is shown. A modal update dialog with download progress bar is presented; the download-and-install flow is identical to the splash path (Popen installer → 0.5s sleep → `os._exit(0)`)
...
- `--beta` flag: `python build.py 1.0.40 --beta` adds " Beta" suffix to GitHub release title, RTDB `display_version` field, and build summary output — but keeps all filenames clean (no " Beta" in EXE, installer, ZIP, or RTDB `version` field). `write_version_to_rtdb()` writes `display_version` to both `versions/{version}/` and `versions/latest` nodes. `public/index.html` reads `display_version` from RTDB for the download button
