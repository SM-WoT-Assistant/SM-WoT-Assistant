...
- Location: `main.py`, `ui_manager.py`, `painter.py`, `painting_palette.py`, `dialog_utils.py`, `map_renderer.py`, `help_system.py`, `ai_webview_gui.py`, `tray_icon.py`, `firebase_groups.py`
...
- `launcher.py` is built as a separate `--onefile` PyInstaller binary (`launcher.exe` in installer.nsi) with `--hidden-import unicodedata`. It is not part of the main spec — built separately via `build.py:_build_launcher()` if the file exists. The launcher uses `tkinter` + `PIL` for splash rendering and `requests` for RTDB version checking, all DLLs extracted to `%TEMP%`
