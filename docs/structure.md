...
├── admin_version.txt           # Plaintext semver for Admin Desktop App (e.g., 1.0.3) — independent of main VERSION; read by admin_app.py:_read_admin_version() and build_admin.py:read_version(), bundled via --add-data; used by firebase_reporter._report_version() for error-report version when admin EXE is frozen (or no VERSION bundled)
├── admin_uk_seed.json          # Seed EN→UK translations for admin app i18n (99 keys, hand-corrected); bundled via build_admin.py --add-data; used as translation source when admin_uk_cache.json is missing (fresh install = zero Google requests)
...
├── build_admin.py               # Standalone build script for Admin Desktop App (admin_app.py); builds --onefile --windowed EXE via PyInstaller, local build only (no GitHub upload, no RTDB versions/admin publish), reads admin_version.txt; includes selenium 4 hidden imports (selenium.webdriver.chrome.webdriver/service/options, selenium.webdriver.common.service/selenium_manager/driver_finder) — required for frozen EXE generation (lazy-import selenium 4); also bundles admin_uk_seed.json + temp_scripts/decoded/tiers_devices_decoded.xml via --add-data and --hidden-import deep_translator for admin i18n EN→UK; run separately via python build_admin.py
...
├── window_manager.py           # Window positioning, click-through, WinAPI, resizing; reads/writes edit_h from settings for height persistence; format_mode_enabled (F8/Lock) allows drag in both norm and edit modes — global drag (курсор поза вікном) лише в norm (click-through, фіча), в edit — тільки коли курсор над вікном програми (_cursor_over_app(), WindowFromPoint hit-test, фікс дрейфу позиції 04.08.2026)
...
├── data_manager.py             # JSON file I/O, tank DB loading with fallback chain (tank_db.json → tank_tth.json → extracted_data/ filenames), each fallback level calls firebase_reporter.report_fallback()
...
├── map_manager.py              # Map list loading (unified TACTIC↔MAPS from extracted_maps/map_dictionary.json), game version check, data updates, TACTIC image existence filter, filters out type/*, invalid_map, hangar_v4, h33_* maps; _EVENT_MAP_IDS (18) — event/onboarding/removed maps (sm24/nom/scc/ls26/epic/BR/wt/140-142; 04.08.2026: 59_asia_great_wall та 37_caucasus прибрані — живі ctf-мапи клієнта); group scheme map filtering — when active_group_id is non-Public, filters map list to only maps that have at least one scheme in the active group; battle mode filtering maps mode map: Standard→ctf, Encounter→domination, Storm→assault, Onslaught→comp7, OnslaughtLight→comp7; TACTIC mode with non-Standard battle mode clears map list and shows "no maps" message via show_main_splash(message=); on game version change runs GameEntitiesExtractor + regenerate_game_entities(None) + TankExtractor.update_compact_descr(); report_fallback() on map_dictionary.json load failure and pending_updates/builds signaling failure
├── map_renderer.py             # Map image rendering, 10×10 grid, draw_frame() border, _resolve_tactic_folder(), arena bases (supports assault2 gameplay type as fallback for Storm mode); show_main_splash(message=None) accepts optional message parameter for "no maps" display in TACTIC mode; displays borderless window warning on editor splash; _report_fallback_async() fires daemon-threaded firebase_reporter.report_fallback() when boundingBox is missing bottomLeft/upperRight in map_data.json
...
├── translations.py             # Default UI/map translations (English-only authoritative source); class labels (LT/MT/HT/TD/SPG) kept in English for all languages; close_with_game key for tray watcher; auto_window_size key for DPI-aware drawing scale checkbox; added sync_schemes_with_mode, select_all, help_select_all, help_size_slider, borderless_warning, size_label, entity_update_done keys; renamed Assault→Storm, added OnslaughtLight, Onslaught " 10"/OnslaughtLight " 8" suffixes
├── stats_ai.py                 # AI tank build browser (Treeview + Firebase RTDB sync), popular tanks (tiers 8-11), version-tracked caches, Firebase _sync_popular_tanks() and _sync_builds(); _report_fallback() deduplicated fallback reporting via _REPORTED_FALLBACKS set; icon resolution cross-category fallback (searches other category dirs when icon not found in own category); _back_from_detail() and back button (chr(0xF112) FontAwesome) in SETUP tank detail view; guard against active_tank=None in detail rendering
├── stats_data.py               # Equipment, consumable, crew skill ID mappings; generates EQUIP_MAP/CONS_MAP/CREW_SKILL_MAP at runtime from .mo files via generate_mo_maps() (called from language_module.setup()), caches per language to mo_maps_{lang}.json; replaces old hardcoded mapping dicts; uses game_entities_english.json for category membership
...
├── generate_prompt_v2.py       # AI prompt generator for competitive tank builds (admin-side only, used by admin_build_generator.py); loads equipment data dynamically from tiers_devices.xml via parse_tiers_devices.load_tiers_devices() with hardcoded _FALLBACK_BY_TIER/_FALLBACK_EXCLUDE_BY_CLASS fallback + firebase_reporter.report_fallback() on failure; _cons_rev_map() reverses ai_engine.AI_CONS_MAP (the same dict the answer parser uses) instead of stats_data.CONS_MAP — gives English ration names for all 11 nations in Main/Advanced loadouts (National Ration in Advanced loadout slot 3); removed get_field_mods() hardcoded list
...
├── tank_extractor.py           # Tank data extraction from game client; update_compact_descr() populates missing compact_descr fields in tank_db.json from versioned nation folders (china_4/, usa_9/); nation folder name handles version suffix (_\d+$) for nation resolution
├── wot_decoder.py              # XML decoder for game client .pkg files
├── decode_xml.py               # XML decoding utilities (bounds-checked — guards against malformed data with offset/child_count/id bounds)
├── decode_xml.orig.py          # Pre-fix backup of decode_xml.py (prior to bounds-check additions)
├── debug_decode_xml.py         # Debug/test harness for decode_xml.py
...
├── parse_game_entities.py      # Game entities parser; extracts field mods from extracted_data/common/post_progression/field_modifications.xml with hardcoded fallback; parses crew perk names from .mo at runtime (no hardcoded perk map); bounds-checked BWXmlDecoder
...
├── parse_tiers_devices.py      # Equipment tier/device parser from tiers_devices.xml; used by generate_prompt_v2.py for dynamic equipment lists with hardcoded fallback; first candidate is now the game client {wot_path}/res/packages/scripts.pkg (wot_path from config.SETTINGS_FILE) — read as a ZIP via _try_client_pkg() and cached raw to AppData tiers_devices_raw.xml, then _ensure_decoded() writes decoded output to config.USER_DATA_DIR/tiers_devices_decoded.xml (writable; old base_dir/temp_scripts/decoded/ path no longer read)
...
├── language_module.py          # .mo file parser, game language detection, locale JSON exporter for website; global _lang_module via get_lang_module()/reset_lang_module(); regenerate_map_dictionary() and regenerate_game_entities() on language change; setup() calls stats_data.generate_mo_maps(src, lang) with src = _lang_module if set else lm — the cached _lang_module is guaranteed populated so mo_maps are never regenerated empty
...
├── firebase_reporter.py        # RTDB REST: error reports (`report_error(type, stage, error_text, blocked=False, details=None)` — structured signature with error types including `bundle_integrity`, `install_marker`, `verify_failed`, `game_client`, `fallback`; `blocked=True` for fatal startup errors), `report_fallback(source, context, error_text, level)` wrapper for non-fatal data fallback events; `_report_version()` resolves the report version — reads `admin_version.txt` from BUNDLE_DIR when the admin EXE is frozen or no VERSION is bundled, else `config.load_version()`; version pings, update checks, service events; compare_versions() strips " Beta" suffix before semver comparison
...
├── admin_build_generator.py    # Admin daemon (--listen) polls pending_updates, generates all builds via Gemini, publishes to RTDB; manifest fingerprinting helpers: detect_changed_tanks(wot_path, manifest_path) compares scripts.pkg vehicle XML fingerprints to the manifest, snapshot_manifest(wot_path, manifest_path) writes a baseline, update_manifest_for_tags(wot_path, manifest_path, tags) advances entries for generated tags; _MANIFEST_LOCK (threading.Lock) guards detect reads + snapshot/update writes
├── admin_app.py                # Desktop admin tool (tkinter GUI) for build generation, scripts.pkg scanning, WG API game version monitoring, auto-detect changed tanks, generate builds via Gemini, tray icon with balloon notifications, settings persistence, --tray CLI for minimized start; version read from admin_version.txt (shown in window title + Admin Version card); single-instance mutex SM_WoT_Assistant_Admin_SingleInstance restores the existing window on second launch; _log() appends to admin.log in %APPDATA%\SM WoT Assistant and reverses the UI log — new messages inserted at "1.0" (top) with auto-scroll see("1.0"), capped at 1000 widget lines via delete("1000.0", tk.END); right-click (and help text) opens Copy/Select All context menu (_copy_selection); i18n EN/UK — authoritative _TR_EN dict (99 keys), self.t(key, **fmt) returns current language from admin_settings.json lang key (default "en"), EN/UK toggle button (_toggle_lang) calls _apply_lang() to update _tr_widgets; Ukrainian translations cached in %APPDATA%\SM WoT Assistant\admin_uk_cache.json (en_snapshot/uk/updated_at) — loaded via _load_uk_translations(): on en_snapshot match uses cache without Google requests, else translates only changed keys via deep_translator.GoogleTranslator (_translate_en2uk, fallback to English); admin_uk_seed.json bundled as fresh-install source; _shield()/_unshield() protect {n}/{path} placeholders and tokens (SM WoT Assistant, scripts.pkg, WoT, WG, AI, F1, Ctrl...) from Google mangling; F1 help (_show_help) — dark Toplevel 680×560 read-only ScrolledText with _help_text() sections (Buttons, Settings, Tray, Background automation, Log, F1), same dialog from ⚙ → Help; no auto-update (RTDB versions/admin check removed); WoT path auto-detect (_resolve_wot_path): CLI --wot-path → admin_settings.json → main %APPDATA%/SM WoT Assistant/settings.json wot_path → common paths, validated via version.xml, saved back to admin_settings.json; change-tracking manifest (_resolve_manifest) lives in %APPDATA%\SM WoT Assistant\.tank_extract_manifest.json (NOT CWD — frozen onefile CWD is %TEMP%\_MEIxxxxx), seeded from a fresh dev manifest or snapshot_manifest() baseline of scripts.pkg so first scan reports 0 changes; all detect_changed_tanks() calls pass manifest_path=self._manifest_path; update_manifest_for_tags() advances manifest entries for generated tags after ok=True so tanks don't re-detect every 60-min scan (guarded by _MANIFEST_LOCK); RTDB status publishing (_report_admin_status) — writes admin_app/ node: version + last_seen epoch heartbeat every 60s, status idle/generating/offline; _do_generate additionally writes admin_app/last_generation {at, count, ok}; success writes builds/prompts/last_generated_at (popular_tanks/last_generated_at on _do_popular), and _do_generate writes pending_updates/builds generating/done/error; _exit_app writes status=offline (daemon thread, non-blocking); _cleanup_old_error_reports() deletes error_reports older than 60 days (REST endAt query + PUT null per key, parseable timestamps only), runs on first _start_background iteration + every 24h, logs via log_cleanup_done key; ⚙ gear button opens _show_settings_menu() dropdown (tk.Menu tk_popup): Start with Windows / Start minimized to tray checkboxes + WoT Path... dialog (_show_wot_path_dialog) + Exit — the only full-exit path; X button (_on_close) minimizes to tray via root.withdraw() (background loop keeps running); tray wndproc is a pure forwarder — LMB (0x0202) or double-click (0x0203) → deiconify()+lift() only, no context menu (menu attempts crashed with abort() 0xC0000409 in ucrtbase from reentrant Tk calls); "Running in tray (WoT: ...)" balloon at start in tray; built as standalone --onefile EXE via build_admin.py
...
│   ├── admin.html              # Admin dashboard (stats, schemes management, version management, user management with PIN reset, error reports with desktop notifications, pending builds status polling); live Admin App (Desktop) section reads the admin_app/ RTDB node — Admin Version, Status (Online if last_seen < 5 min), Activity (Idle/Generating), Last Generation, Builds/Prompts/Popular updated timestamps — via loadAdminAppData() polled every 60s; desktop admin EXE distributed separately as local build only via python admin_app.py
...
│   ├── map_dictionary.json     # Internal key → display name у мові клієнта гри (регенерація з arenas.mo на кожному старті, language_module.setup)
│   └── *.png                   # Map images
...
├── temp_scripts/               # Temporary extraction scripts (scripts/ with arena_defs/, common/, entity_defs/, item_defs/, space_defs/; decoded/ + decoded_test/ hold decoded XML output)
