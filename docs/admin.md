...
- **Tab-ізоляція (v1.0.24, 21.08.2026):** `_fetch_yt_chrome`/`_fetch_reddit_chrome`/`_fetch_kofi_chrome` працюють у НОВІЙ фоновій вкладці — вкладка юзера ніколи не навігується; v1.0.26: вкладка створюється через CDP `Target.createTarget` (`_open_bg_tab`, БЕЗ активації в UI — close не перемикає видиму вкладку; fallback `new_window`); close-guard: закриває фонову вкладку тільки якщо вкладок > 1 (закриття останньої змушує Chrome перестворювати вікно — корінь «іконки в таскбарі»)
