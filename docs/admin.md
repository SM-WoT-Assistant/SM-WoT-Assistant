...
- **Вибір вікна (v1.0.21):** `_find_hwnd_by_pid` вибирає НАЙБІЛЬШЕ видиме вікно pid за площею (GetWindowRect) — модальний діалог Chrome ніколи не переможе головне вікно; pid береться з `drv.service.process.pid` (Selenium, без PowerShell); `_chrome_main_pid` — лише fallback
