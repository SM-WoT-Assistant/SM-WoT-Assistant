...
## ������� FAILED ����� � 5 ������� + �������� ������ ��������� (13.08.2026, ������ v1.0.15)

1. **generate_builds > (ok, done_tags, reasons)**: reasons = `{tag: "��������: ������"}` ��� ������� ��������� ����� + `reasons["summary"]`. ������� ����: `unknown_tank` / `client_parse_fail` / `no_prompt` / `ai_no_response` / `ai_parse_fail` / `upload_fail`. ������: ���������� �����, admin.log + GUI-��� + ����-����� (������ ������ ���������� FAILED�), RTDB `pending_updates/builds` (status=error, message=summary), `report_fallback` > `error_reports` (admin.html Errors).
2. **Fail-�����** � `.tank_extract_manifest.json` (`_failures: {tag: {count, fp}}`): `update_manifest_failures()` / `exclude_failed_tags()`, ���� 3 > ���� ����������� � ������� � ��������� �����; �������� ��������� � ������ ��������� (����� `update_manifest_for_tags`) ��� ���� ������������ ���� scripts.pkg-����� (����� ���� ��� = ���� ������). ³��� ����� silent FAILED ���������.
3. **������ �����**: `detect_changed_tanks` ��������� ��� ���������� scripts.pkg (`report_fallback`) ������ ������ `[]`; `_tank_record_from_client` ����� `RuntimeError("list.xml parse failed: ...")` ������ ������ `None`; `_fill_progress.json` > `config.USER_DATA_DIR` + ����� `start_idx >= len(all_tags) > 0`; `builds/version` ��������� ���� ��� `ok_count > 0` (����� � `_do_generate` ��������); ���� ����������� ����� `exclude_failed_tags`.
4. **�������� � ����**: Chrome ������ ��-���� (`--window-position=-32000,-32000` ������ `--start-maximized` � ������� ���� ������ �����); ������ ������ � ���������� ��� (`start_minimized: True` ��� ����� ����������, X > � ����, ����� ����� Settings); dev-������� � `pythonw.exe` (��� ����������� ����; ������� ����� � ��� � admin.log + RTDB).
5. **����������� (#1471)**: ast 4 ����� + 8 ����������� ����-����� fail-������ + live-����� (RTDB `error_reports` �����, Chrome ������� `(-32000, -32000)`), ������ v1.0.15 � ��� ������: 995, �������: 996�, ���� �� ��������.
