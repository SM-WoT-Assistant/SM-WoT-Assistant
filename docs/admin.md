...
- **Reddit** (v1.0.35, 24.08.2026): **ТОЛЬКО сесія браузера, без автологіну** (`_fetch_reddit_browser`) — фонова вкладка → `submitted.json` → парс JSON з `<pre>`; 0 постів + CAPTCHA-маркери → статус `blocked` (ручний логін бота у вбудованому браузері), інакше `empty`. Історія: публічний .json з UA = 403 з 2026; vault-автологін `_login_reddit` ВИДАЛЕНО — він став причиною прапорця «automated activity» на акаунті. Анонімний доступ CAPTCHA-гейтований, тому без сесії даних нема (це норма, не баг)
...
- **Кеш + оновлення (v1.0.28, 21.08.2026; змінено v1.0.36-v1.0.38, 24-25.08.2026):** `community_cache.json` (AppData) — кеш даних платформ з валідацією; при старті кеш завантажується (плитки миттєво). **Вхід у Community = HTTP-only refresh** (`_refresh_community_background(http_only=True)` — RTDB-лічильники/Errors/GitHub/Patreon/YT-API; НУЛЬ фонових вкладок — раніше повний refresh при вході відкривав 3 CDP-вкладки, що вискакували у стрічці вкладок браузера при розгортанні). **Повний шлях (v1.0.37) — перший прохід фонового циклу при старті + цикл 12 годин (43200с): фетчить УСІ платформи включно з Reddit/Ko-fi/YT-fallback і в треї** — Chrome стартує офскрін. **v1.0.38 (25.08, рішення юзера): резидентність прибрана — після завершення будь-якого фонового refresh браузер зупиняється (`_community_kill_browser()` у finally), якщо Community не у fullscreen (`fs`)**; інцидент-причина: офскрін-вікно на -32000,-32000 показувалось у таскбарі як недосяжна «закладка» (12:45-12:55 25.08, до падіння ОС). Ціна: наступний фетч/вхід = холодний старт драйвера ~3-5с; статус "hidden" не виставляється (на вході статус браузерних платформ не перезаписується). Вручну — Refresh/«Зв'язати». Технічні цикли без змін (heartbeat 60с, 24h cleanup/sweep, WG-версія 30 хв). При вході завжди вкладка Overview. ensure_browser НЕ вбиває драйвер при visible-mismatch (критичний фікс kill-race)
- **Плитка Reddit** (v1.0.26): кількість постів + статус-хінт, клік → вкладка Reddit; tile_reddit i18n
- **Tab-ізоляція (v1.0.24-1.0.27, 21.08.2026):** `_fetch_yt_chrome`/`_fetch_reddit_browser`/`_fetch_kofi_chrome` працюють у НОВІЙ фоновій вкладці — вкладка юзера ніколи не навігується; v1.0.27: `_open_bg_tab` (метод) — CDP `Target.createTarget` + **`Target.activateTarget(main)`** (UI гарантовано повертається юзеру навіть якщо Chrome активує нову вкладку; `[DEBUG][bgtab] cdp=ok/fail` в admin.log; fallback `new_window` + `about:blank` — без стартової сторінки google); close-guard: закриває фонову вкладку тільки якщо вкладок > 1
...
- **Watcher-embed + TOOLWINDOW (v1.0.25, 21.08.2026):** `_community_poll_embed` — постійний монітор (перестворене вікно Chrome негайно вбудовується знову); `_start_toolwindow_guard` (daemon, 3с) застосовує `WS_EX_TOOLWINDOW` до ВСІХ top-level вікон pid → жодної іконки Chrome в таскбарі ніколи; `tg_started` скидається при скиданні драйвера. **v1.0.35:** SetFocus на кожен полл поки Chrome видимий (фокус завжди у браузері — Ctrl+V/ESC працюють без наведення); додаткові top-level вікна pid (CAPTCHA-попапи) вбудовуються у фрейм теж (`embeded_windows`, розгортаються назад у offscreen при виході); кнопка **✕** у шапці fs + **Alt+F4** → `_exit_community` (підміна/відновлення `WM_DELETE_WINDOW` через `_fs_prev_close`)
...
- Файл `%APPDATA%/SM WoT Assistant/admin_vault.json`: `{"youtube": {"api_key": "<base64 DPAPI>"}, "kofi": {...}}` — API: `vault_get/vault_set/vault_has/vault_delete_field/vault_delete` (reddit-поля виведені з ужитку v1.0.35 — автологін Reddit видалено; старі значення в файлі ігноруються)
...
- Жива проба Reddit-шляху (v1.0.35): без сесії → статус `blocked` + CAPTCHA-детект; із сесією бота → пости парсяться з `submitted.json`
