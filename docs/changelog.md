...
Перший фікс виявився неповним — юзер побачив: "видно тільки напис Групові схеми, кнопки обрізаються, кнопки Download/Cancel видавлюють меню груп". **Корінь:** `req_h` вимірювався з ПОРОЖНІМ `download_frame` (список будується асинхронно через `after(0)`), а `_build_download_ui` потім додає фільтри + tree + **ряд кнопок Download/Cancel** (~280px) — повний контент перевищує вікно → pack зрізає найнижчі елементи = групове меню. Фікс (painting_palette.py):
- `download_frame` пакуються **після** `group_mgmt_frame` (`after=` замість `before=self._status_lbl`) — при нестачі висоти зрізається низ списку (tree скролиться), кнопки групових схем (Create/Join/Manage) видимі на будь-якому екрані/DPI
- новий хелпер `_resize_for_download()` — висота за повним контентом `min(max(reqheight, 780), screenheight - 120)` + **вікно піднімається**, якщо `y + target_h > screenheight` (низ ніколи не за екраном); викликається при відкритті та `after(50)` після `_build_download_ui` (контент готовий)
- **Верифікація:** емпіричний тест у пам'яті (реальна палітра + mock-список з 20 рядками + кнопки Download/Cancel): `groups visible: True (645 <= 917)`, download у межах вікна, вікно в межах екрана, компактна геометрія відновлюється при закритті
