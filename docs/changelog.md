...
## SETUP: блок ТТХ — ширина + автоширина назв + КОРІНЬ зникнення (28.08.2026)

Юзер: «у розділі сетап треба розширити зону з написами ТТХ десь на 100 пік... треба зробити щоб ТТХ з зображенням танку було такої ж ширини як і нижній блок».

**Коміти:** e42970e (fill="x") → 028bf89 (width=max(frame,440)+Configure) → 6b9df34 (автоширина назв: знято width=15) → 954ef68 (ФІНАЛЬНИЙ, grid).

**Епопея зникнення (2 раунди «блок пропав»):** [TTH-DEBUG] вимірами встановлено: `late: frame=600x1 wrapper=600x1 table=388x192` — wrapper мав висоту **1px** при живому контенті (таблиця 192px). **КОРІНЬ: pack заморожує парсел на момент пакування.** Рендер (`_finish_tank_detail`) будує блок, коли кадр ще НЕ розмірений (frame_w=1, unmapped) → pack розклав wrapper у парсел 1×1 → кадр пізніше виріс до 600×204, а pack ніколи не перерахував wrapper (хитання ev_h 1↔204 = бій pack↔grid). Фікс: **wrapper у grid sticky nsew** (не pack!) — grid автоматично слідкує за клітинкою: ширина = ширині кадра (= нижній блок), висота = контенту. Configure-хендлер/_tth_wrapper видалені як мертві. Верифікація: репліка «unmapped→grown» (wrapper 600×162 повний), 0 залишків, compile, жива перевірка юзера «Все Ок!».

**Підсумок змін (stats_ai.py):** `_detail_tth_fixed_width` 250→**350**; назви — автоширина (було width=15, обрізало українські назви); блок = ширині нижнього блока на будь-якій ширині вікна.

**УРОК (durable):** пакувати pack-ом у НЕвідображений кадр (1×1) = назавжди заморожений парсел; grid sticky nsew не має цього бага — для динамічних контейнерів, що будуються до маппінгу, використовувати grid. (див. пам'ять сесії)

---

## Конвеєр даних: санітайзинг декодера + фільтр івент-здібностей (28.08.2026, Фаза 2)

Юзер (термінал): `[ПОМИЛКА] ET ParseError: not well-formed (invalid token): line 11, column 16` на `optional_devices.xml` + «Іконки не знайдено: 33». Юзер припустив: «комп7 здається це якісь івентні елементи які ми відфільтровуємо?» — підтверджено даними клієнта.

**Корінь 1 (ParseError):** декодер `decode_xml.WotXmlParser` емітує XML-невалідні контрольні символи з бінарних значень — свіже in-memory декодування pkg-запису дало 29 символів (0x0B у `<crystal>`, 0x13, 0x0F...). Це ламало ET.parse → game_entities.json зберігався БЕЗ deluxe/experimental обладнання (тиха втрата). Фікс: `sanitize_xml_text` (decode_xml.py) на виході decode_file + на вході parse_xml_file — вбиває клас назавжди, лагодить застарілий файл без ре-екстракції.

**Корінь 2 (33 іконки):** клиент маркує comp7_*/poi_* як `<type>battleAbilities</type>` + notForSale + roleEquipment/poiEquipment (Onslaught-здібності, НЕ обладнання), але extract_equipment_from_xml класифікував їх як «standard» і клав у game_entities (159 записів у equipments-1!). Фікс: пропускати battleAbilities (клієнтська класифікація, без хардкоду імен; прецедент — generate_prompt_v2:422 skip comp7_). Плюс нормалізація path-style icon (`../maps/icons/artefact/turbocharger.png` → `turbocharger`, новий формат WG).

**Результат перегенерації** (python parse_game_entities.py): optional_devices.xml парситься (89 пристроїв), equipment 210→140 (реальне обладнання), іконки 33→1 (лишився radioman_lastEffort — іконки НЕМАЄ в жодному pkg клієнта, інформаційно; turbocharger ×2 виправлено нормалізацією). game_entities.json перегенеровано (вона в _CRITICAL_JSON бандла — важливо до релізу).

Верифікація: py_compile + реальні методи (decode_file→parse_xml_file→extract_equipment_from_xml): 10/10 (0 контрольних символів у декоді, stale-файл парситься, 0 comp7/poi у результаті, lastEffortBattleBooster/реальне обладнання на місці, 51=29 eq+22 consumables). Docs: game-data.md секція «Санітайзинг декодера + фільтр battleAbilities».

---

## Малювання: фільтри-теги замість значків + буфер обміну + чекбокси режимів геть (27.08-28.08.2026, кроки 1-3)

Юзер (26.08): «Чекбокси на панелі малювання додавали значки техніки при малюванні елементів - це треба відключити а замість цього вибраний чекбокс має фільтрувати схеми за класами техніки... Чекбокси режимів бою на палітрі малювання здається не потрібні бо перед тим як малювати схему ми вибераємо мапу у вікні редагування і тим самим вибираємо і режим бою».

**Контекст:** перша спроба (7fa275f/ab91000) була відкочена через крах старту (keysym `cyrillic_es` у bind() — Windows Tk відкидає) та накладені помилки; задача перероблена покроково по команді юзера з верифікацією кожного кроку.

**Крок 1 (5967e9a):** значки класів видалені скрізь (рендер ×3, `_draw_class_icons`, `class_icon_codes`, `_get_marker_class_anchor`, hit-test/drag-гілки, coords-обчислення в палітрі ×2); `is_visible` без гейту; налаштування «Sync schemes with battle mode» видалене (чекбокс+var+save+2 ключі; автосинк перед боєм — окремі чекбокси). Верифікація: таблиця істинності 13/13 на реальному коді (порожні теги = всюди; HT-тег ⇔ HT у фільтрі; Storm-тег ⇔ радіо Storm; UK-легасі; перетин).

**Крок 2 (06b6277):** Ctrl+C/Ctrl+V через `<Control-KeyPress>` + keycode (VK 67/86) — розкладконезалежно, без keysym-імен (урок краху 26.08); бінди на root + обидва канваси + палітру; `"break"` без подвійного спрацювання; гард текстових полів; статус-підказки (4 нові ключі). Верифікація: реальний Tk-бінд прийнятий + event_generate + таблиця 8 станів.

**Фікс 1 (f18e245):** регресія `on_press` — розвкладений select-all блок давав UnboundLocalError: palette на кожному кліку; повернено baseline-вкладеність. (LSP «palette is possibly unbound» був реальним попередженням — тепер звіряється з baseline.)

**Фікс 2 (8a58061):** буфер мовчазно не спрацьовував при фокусі на оверлеї `_po_win` (bindtags без root) — додано бінд на Toplevel оверлея + тимчасові [CLIP]-принти.

**Фікс 3 (3beb952):** [CLIP] довів: реальний Windows Tk дає `state=12` (Control + біт 0x8) для звичайного Ctrl+C/V — гард `state & 0x8` (помилково прийнятий за Alt) блокував УСЕ; гард знято. Верифікація рівно продакшн-значеннями (state=12/268, keycode 67/86).

**Фікс 4 (e7cf824):** паста «в інший режим вставляє, на поточну мапу ні» — (1) успадкування modes від оригінала ховало копію при іншому радіо → паста тепер штампує теги з поточних чекбоксів палітри; (2) `_strip_duplicates` мовчазно зрізав тотожну копію → паста зберігається через `data_mgr.save_drawings` без dedup.

**Фікс 5 (8a284ba):** копія лягала РІВНО на оригінал (невидима) → зсув +20px каскадом (coords + text_coords, `_paste_count` скидається на copy).

**Крок 3 (ce7ee19):** чекбокси режимів палітри ВИДАЛЕНО (mode_vars/mode_labels/UI-блок/`_mode_mo_keys`/select-all-штамп/load_object/`_write_to_object`); паста `modes=[]` (універсальна); мертвий `battle_mode` ключ ×2 видалено (`battle_mode_label` лишився); мертвий import language_module з палітри знято; [CLIP]-принти прибрані (підтверджено живцем).

**Хелп (F1):** додано рядки Ctrl+C (копіювати вибраний елемент) / Ctrl+V (вставити копію +20px, класи з чекбоксів палітри); описи фільтрів оновлені (завжди активні; чекбокси палітри тегують елементи).

Верифікація кожного кроку: py_compile + import-smoke + headless-тести реального коду (is_visible 13/13; паста 9/9 + 8/8; крок 3 — 7/7; on_press runtime) + жива перевірка юзера. Docs: painter.md секція «Фільтри/буфер — фінальний стан».

---

## Адмінка v1.0.38 (25.08.2026): без резидентного офскрін-Chrome у треї (фантомна «закладка» в таскбарі)

Юзер: «декілька годин назад на панелі завдань я побачив закладку з хромом яку не можна було відкрити - таке ставалось коли ми налаштовували адмінпрограму і що дивно що не були завантажена ні основна програма ні адмін програма».

**Розслідування (усі факти з доказів):**
1. Адмінка v1.0.37 (зібрана 25.08 00:32) жила в треї з 00:45; о **12:45:18** спрацював 12г-цикл Community → офскрін Chrome стартував (`[12:45:18] Community: browser started`), фетчі ok 12:45:42, кеш збережено. За дизайном v1.0.37 браузер **лишався резидентним** — вікно припарковане на `-32000,-32000`, тому клік по його кнопці в таскбарі нічого не показує («не можна відкрити»). Це і був фантом.
2. О **12:55:57** впала сама Windows (EventLog 6008 «завершення роботи о 12:55:57 було неочікуваним» + Kernel-Power 41; WER-крешів застосунків нема). Все вбито; після ребуту автостарт має лише трей-вотчер → «ні основна ні адмін програма не завантажені». Хронологія підтверджена: admin.log/community_cache.json mtime 12:45:42, профіль Chrome 12:48:52, трей-вотчер створений 13:10:54.
3. chrome.exe у процесах з 13:45 = власний Chrome юзера (реальний профіль), до інциденту не стосується.

**Фікс (варіант A, схвалений юзером):** `_refresh_community_background._work` → finally, після всіх існуючих кроків: `if not self._community.get("fs"): self._community_kill_browser()`. Після КОЖНОГО завершеного фонового refresh браузер зупиняється, якщо Community не у fullscreen — невидимого Chrome з недосяжною кнопкою більше не існує як клас. Це скасовує рішення v1.0.37 «лишається резидентним» (нове рішення юзера 25.08). Ціна: наступний фетч/вхід у Community = холодний старт драйвера ~3-5с. Безпека: `fs=True` ставиться синхронно ДО запуску потоків (`_enter_community`), kill уже викликається з фонового потоку в `_reset_browser_data` (прецедент), подвійний kill із `_on_close`/`_exit_app` ідемпотентний.

Верифікація (#1471): baseline py_compile OK; після правки py_compile OK; AST-smoke — finally закінчується умовним kill, docstrings оновлені, `_community_kill_browser` не пошкоджений (driver/tg_started/log на місці), точки виклику 1798/2347(new)/3045/3054, симуляція умови на 4 станах (fs False/True/відсутній/None). `admin_version.txt` 1.0.37 → **1.0.38**, rebuild build_admin.py.

---


## Бойове вікно: прозоре для кліків під замком, розблокування Ctrl+клік по замку (24.08.2026)

Юзер: «Треба зробити бойове вікно прозорим для клику миші коли вікно заблоковане на замок - щоб натиснути на замок треба зажати Контрл» (тільки бойове вікно, edit не чіпали).

**Було:** замок (формат OFF) у norm → `set_clickthrough(True)` на корінь → всякий клік іде в гру, кнопка замка недосяжна, розблокування тільки F8.

1. **`window_manager.py`:** `set_clickthrough()` запам'ятовує стан у `self._ct_transparent` (init None); новий `_update_lock_clickthrough(ctrl_pressed)` викликається з існуючого 60FPS-потоку `_monitor_mouse_drag` (нуль нових потоків): активний ТІЛЬКИ при формат OFF + mode="norm" + не drag + не діалог + вікно видиме; Ctrl затиснута → `set_clickthrough(False)` (вікно клікабельне), відпущена → `set_clickthrough(True)`; стиль міняється лише при зміні бажаного значення. В інших станах не втручається (стилями володіє колишня логіка).
2. **`main.py` toggle_formatting_mode:** гард — розблокування кнопкою замка в norm вимагає фізично затиснуту Ctrl (GetAsyncKeyState VK_CONTROL); F8 без змін.
3. Побічні ефекти: самолікувалась непослідовність «замок через кнопку лишав вікно клікабельним» (поллер вирівнює за ~100мс); з затиснутою Ctrl клікаються всі кнопки топбару бойового вікна; фокус грі при відпусканні Ctrl НЕ повертається автоматично (рішення — менше сюрпризів).

Верифікація: AST OK обидва файли; `import window_manager` OK; grep — поллер визначений+викликається, стан-трекінг на місці. Жива перевірка за юзером.

---


## Адмінка v1.0.37 (24.08.2026): Reddit/Ko-fi оновлюються при старті програми (Chrome офскрін у трюї)

Юзер: «тепер треба редіт і кофі оновлювати при старті програми» + «лиши» (на питання про резидентний Chrome).

**Корінь:** повний refresh на старті спрацьовував, але гейт `_app_visible()` пропускав Reddit/Ko-Fi поки застосунок у треї (політика «no Chrome in tray»). Оновити їх без браузера неможливо — обидва джерела вимагають cookie-сесію.

**Зміни (`_refresh_community_background._work`):**
1. Гейти браузерних блоків `not http_only and self._app_visible()` → просто `not http_only` (yt-fallback / reddit / kofi). Повний шлях (старт + 12г) тепер фетчить браузерні платформи і в треї.
2. Прибрано else-гілки `status="hidden"` — на вході (http_only) статус Reddit/Ko-fi більше не перезаписується на «Не підтверджено», лишається останній реальний («ok»/«blocked»).
3. Chrome стартує офскрін при старті адмінки і лишається резидентним (рішення юзера) — перевикористовується 12г-циклом і входами в Community; toolwindow guard як і раніше не дає іконки в таскбарі.
4. Мертва `_app_visible()` видалена (після зняття гейтів не мала викликів); docstring `_community_ensure_browser` оновлено (політика «no Chrome in tray» скасована).

Верифікація: import-smoke OK; grep — 3 гейти `not http_only`, `_app_visible` = 0 посилань. Інцидент: перший запис admin_version.txt не виконався (bath-виклик прогнав лише bash) → білд пішов з лейблом 1.0.36 на коді 1.0.37 → перезаписано, перезбірка коректна. `admin_version.txt` 1.0.36 → **1.0.37**, rebuild (8.6 MB, _internal 69).

---

## Адмінка v1.0.36 (24.08.2026): вхід у Community без фонових вкладок (HTTP-only refresh)

Юзер: «треба щоб інформація по вкладках оновлювалась при старті програми або кожні 12 годин — щоб при розгортанні розширеного вікна не вискакували закладки у браузері різних програмних закладок».

**Корінь:** `_enter_community` запускав повний `_refresh_community_background()` при КОЖНОМУ вході → 3 фонові CDP-вкладки (YT watch / Reddit submitted.json / Ko-fi Manage) з'являлись і зникали у стрічці вкладок вбудованого браузера під час розгортання.

**Фікс (варіант B):**
1. `_refresh_community_background(http_only=False)` — новий прапорець: `http_only=True` фетчить ТІЛЬКИ HTTP-джерела (RTDB-лічильники, Errors, GitHub, Patreon, YT через API key) — нуль дотиків до браузера.
2. Гейти `not http_only and self._app_visible()` навколо трьох браузерних блоків: `_fetch_yt_chrome`, `_fetch_reddit_browser`, `_fetch_kofi_chrome`.
3. Вхід у Community → `_refresh_community_background(http_only=True)` — плитки свіжіють тихо; розгортання більше НІКОЛИ не відкриває вкладок.
4. Браузерні платформи (Reddit/Ko-fi/YT-fallback): старт програми (перший прохід фонового циклу, `last_comm=0`), цикл 12 годин, ручні Refresh/«Зв'язати» — без змін.

**Свідомий трейдоф:** якщо адмінка живе в трюї і 12г-тік станеться поза Community — Reddit/Ko-fi оновляться тільки вручну (плитка покаже кешоване число). Розширення варіантом C (точкове автооновлення застарілої вкладки при її відкритті) можливе пізніше як надбудова.

Верифікація: import-smoke OK; grep — рівно 2 точки виклику (вхід http_only=1, цикл повний), 3 гейти на місці. `admin_version.txt` 1.0.35 → 1.0.36, rebuild (8.6 MB, _internal 69).

---


## Адмінка v1.0.35 (24.08.2026): Reddit — тільки сесія браузера (без автологіну), embed-фікси, ✕/Alt+F4 у Community

Контекст: Reddit-акаунт проєкту отримав прапорець «automated activity» — причиною була форма-автологін з vault (`_login_reddit`: fill username/password + submit). Рішення: програма НІКОЛИ не логіниться сама — читає публічний профіль через існуючу сесію браузера (ручний логін бота).

**Reddit rework:**
1. Видалено 4 функції: `_fetch_reddit_http` (анонімний .json — 403 з 2026), `_reddit_logged_in` (чек /settings/), `_login_reddit` (vault-автологін — корінь проблеми), `_fetch_reddit_chrome` (оркестратор з логіном). Мертва `_parse_reddit_html` теж видалена.
2. Нова `_fetch_reddit_browser()`: їде на сесії постійного профілю, БЕЗ логіну; фонова вкладка → `submitted.json` → парс JSON з `<pre>` (той самий доведений формат); 0 постів + CAPTCHA-маркери на сторінці → статус `blocked` (червона дія «залогінься вручну»), інакше `empty`.
3. Диспетчерські точки: `_refresh_community_background` (гейт `_app_visible`, hidden → "hidden"), `_community_refresh_tab("reddit")`, `_community_link_platform("reddit")` — один виклик без фолбеків.

**Embed-фікси (`_community_poll_embed`):**
4. Фокус: `SetFocus(chrome)` на КОЖЕН полл поки Chrome видимий — прибрано гейт `_cursor_over_hwnd` (наведення без кліку не повертало фокус після `focus_force()` при вході в fs; Ctrl+V/ESC «иноді не працювали»).
5. Мультивіконність: додаткові top-level вікна Chrome pid (CAPTCHA-попапи) вбудовуються у `_browser_frame` теж (`_top_level_windows` + IsWindowVisible + GetWindowRect-розмір, трекінг `embeded_windows`) — раніше попап «блимав» поза фреймом і був недосяжний (#1733).
6. Очищення: `_community_move_browser_offscreen` розгортає extra-вікна назад у offscreen top-level; `_community_kill_browser` скидає `embeded_windows`.

**UI:**
7. Кнопка **✕** у шапці fs (hdr, праворуч) → `_exit_community`.
8. **Alt+F4** у fs → `_exit_community` замість close-to-tray: `_enter_community` підмінює `WM_DELETE_WINDOW` на `_fs_on_close` (зберігаючи оригинал у `_fs_prev_close`), `_exit_community` відновлює.

**Чистка:** i18n `-key_reddit_user -key_reddit_pass -act_login_reddit`; `h_comm_login_d` переписано; поля Reddit у вкладці API Keys видалено; мертвий `_cursor_over_hwnd` видалено (після зняття гейту). Vault reddit username/password більше ніде не читаються (файл admin_vault.json не чіпається).

**Верифікація:** AST OK; import-smoke OK (методи _fs_on_close/_on_close/_fetch_reddit_browser на місці); grep мертвих символів = 0; 3 точки виклику; kofi/YT/GitHub/Patreon потоки не зачеплені. Жива перевірка CAPTCHA-попапу — за юзером.

**Версія:** admin_version.txt 1.0.34 → 1.0.35, rebuild build_admin.py.
...
**Причина розбіжності 995/996 (підтверджено даними):** `admin_build_generator.load_tank_db()` фільтрує tank_db.json за `bad`-списком (івент-клони: `_cfe`, `_7x7`, `_fl`, `_sh`, `_storymode`…) + tier 1-11 → 995; `load_prompts()` рахує ВСІ ключі prompts_cache.json → 996. Диф = рівно один тег: **`Ch45_WZ_114_CFE_D`** — CFE-клон WZ-114 (база `Ch45_WZ_114` є і в 995, і в RTDB). Клон згенеровано під час інциденту 13.08 (коміт c04e20f, «промпт опубліковано (prompts_cache 996)»), але він відфільтрований з tank_db → невидимий у застосунку → prompt/build = мертві дані, що ламають лічильники.

**Фікс (клас-рішення, щоб розбіжність була неможлива в принципі):**
1. `admin_build_generator.py`: винесено `_BAD_TAG_MARKERS` (єдина точка правди, був інлайн-список у load_tank_db) + `_is_bad_tag(tag)`. `load_tank_db` — той самий фільтр (995 без змін).
2. Запобігання: `detect_changed_tanks()` і `scan_incomplete_builds()` пропускають `_is_bad_tag` теги → івент-клони ніколи не потрапляють у генерацію/свіп, промпти для них не створюються. Нові регулярні танки (як F141_Durendal) детектяться як завжди.
3. Моніторинг `check_prompt_tank_mismatch(tank_db, prompts)` → `(prompts_only, tanks_only)` — диф по ключах. У `admin_app.py`: нова картка **«Tanks / Prompts»** (зелена при співпадінні, **червона** при розбіжності, оновлюється в `_update_cards`); перевірка на старті (`_check_tank_prompt_match` замість голого `log_tanks_prompts`) і після кожного `_do_generate` (finally) — при розбіжності `[ERROR]`-рядки в лог з конкретними тегами (prompt without tank / tank without prompt). i18n: `card_tp`, `log_tp_mismatch`, `log_tp_orphan_prompt`, `log_tp_orphan_tank` (UK-кеш перекладається автоматично).
4. Очистка поточної розбіжності: ключ `Ch45_WZ_114_CFE_D` прибрано з `prompts_cache.json` (996→995); RTDB PUT null (адмін-токен, патерн #1470) для `prompts/tanks/Ch45_WZ_114_CFE_D` + `builds/tanks/Ch45_WZ_114_CFE_D` — верифіковано GET (обидва → null).

**Ctrl+V у вбудованому браузері (корінь):** v1.0.32 (765e157) давав фокус Chrome лише (1) разово через 150 мс після embed і (2) після кліку над браузером у 200 мс-поллі (`GetAsyncKeyState`). Hover+Ctrl+V **без кліку** (або після `root.focus_force()` з `_enter_community`) → ввід іде в Tk root → «іноді не працює». **Фікс:** безперервна асерція — поки курсор над embedded Chrome (`_cursor_over_hwnd`), `SetFocus(hwnd)` КОЖЕН полл (200 мс), клік-гейт і one-shot таймер прибрані. Обмеження: сайти з блоком clipboard-permission не лікуються фокусом.

Верифікація: AST OK; smoke (`_is_bad_tag`: Ch45_WZ_114_CFE_D→True, Ch45_WZ_114→False, F141_Durendal→False; `load_tank_db` 995 + `load_prompts` 995, диф порожній; admin_app імпортується, нові `_TR_EN` ключі на місці); RTDB GET-перевірка після видалення. build v1.0.33 (8.6 MB, _internal 69) через TMP-обхід (канонічна папка знову «locked» — інцидент #1546). Юзер має бачити: картка «Tanks / Prompts» зелена 995/995; Ctrl+V у вбудованому браузері працює без кліку (наведення курсора достатньо).
...
**YouTube (корінь):** `_navigate_platform_tab` для youtube: без `channel_id` → головна `youtube.com` (не канал!), з `channel_id` → `channel/{id}/videos`; кнопка «YouTube» в Overview вела на `watch?v=4JlDkM65PxY` (відео, не канал).

**Фікс:**
1. `_YT_CHANNEL_URL = "https://www.youtube.com/@SMWoTAssistant"` — константа каналу.
2. `_navigate_platform_tab` youtube → **завжди** `_YT_CHANNEL_URL` (хендл стабільний, не залежить від fetch channel_id; гілка channel_id і fallback на головну прибрані — жодного автоплею відео).
3. Кнопка «YouTube» в Overview (`_link`) → `_YT_CHANNEL_URL`.
4. Фетч-логіка `_fetch_yt_chrome`/`_fetch_yt_api` НЕ чіпана (працює у невидимій фоновій вкладці).

**Reddit (пояснення, не баг):** `empty` — це чесний стан «0 публікацій у профілі SM-WoT-Assistant» (лог: `[DEBUG][fetch] tab=reddit -> empty`, `[DEBUG][link] reddit -> empty linked=True`). З v1.0.30 плитка не показує хінт помилки для `empty`, вкладка показує «✓ Прив'язано». Після першої публікації статус стане `ok`. **Матеріал для публікації підготовлено:** `reddit_post.md` — додано розділ «📺 YouTube channel: https://www.youtube.com/@SMWoTAssistant» (поруч із відео-оглядом); PNG-скріншоти вже готові в `%TEMP%\opencode\reddit_upload\` (8 файлів, конверсія webp→PNG зроблена 17.08, правило #1565). Публікація — вручну в браузері (Reddit API відмовив, #1587).

Верифікація: AST OK; smoke 3 сценарії (youtube без channel_id → `@SMWoTAssistant`; з channel_id → все одно `@SMWoTAssistant`; Overview-кнопка → `_YT_CHANNEL_URL`). build v1.0.31 (8.6 MB, _internal 69) через TMP-обхід; адмінка запущена (15:07). Юзер має бачити: вкладка YouTube → сторінка каналу `@SMWoTAssistant`; кнопка YouTube в Overview → та сама сторінка; плитка Reddit без помилки (empty — просто немає постів, поки не опубліковано).

---

## Адмінка v1.0.30 (21.08.2026): однаковий порядок плиток і вкладок, невидимі фонові вкладки, плитка Reddit без хибної помилки

Юзер: «треба навести лад у плитках і закладках треба поставити їх у однаковій послідовності. На закладці оверв'ю починає завантажуватись адмін сторінка сайту — потім відкривається друга закладка з редіт потім в ній відкривається сторінка ко фі потім закладка вимикається і залишається адмін сторінка — навіщо все це? Ще на плитці редіт позначена помилка — ерор хоча стоїть зелена позначка — Прив'язано!»

**Корінь №1 (різний порядок):** вкладки (`_build_community_ui`): overview, youtube, reddit, github, kofi; плитки (`_build_tile_strip`) були: overview, youtube, github, kofi, reddit — GitHub/Ko-fi/Reddit переставлені.

**Корінь №2 (стрибки вкладок при вході в Community):** `_open_bg_tab` робив `Target.createTarget(bg)` → `activateTarget(main)` → ПОТІМ `drv.switch_to.window(bg)`. Але `switch_to.window()` у Selenium САМ активує вкладку в UI браузера — activateTarget стояв ДО перемикання, а не після, тому фонові фетчі `_refresh_community_background` (reddit_chrome → kofi_chrome) показувалися юзеру: admin.html → вкладка Reddit → вкладка Ko-fi → закриття → admin.html.

**Корінь №3 (плитка Reddit «⚠ error» при «✓ Прив'язано»):** при вході статус reddit = `empty` (0 постів — чесний стан), але `_tile_hint` не знав статус `empty` → остання гілка → `tile_error`. Вкладка ж через `_set_tab_status` (reddit: `empty` у чистих статусах + linked=True) показувала «✓ Прив'язано» — повна суперечність.

**Фікс:**
1. **Порядок плиток = порядку вкладок:** overview, youtube, reddit, github, kofi, installs, errors (перестановка `_tile(...)` викликів).
2. **Невидимі фонові вкладки** (`_open_bg_tab`): порядок змінено — `drv.switch_to.window(bg)` СПОЧАТКУ (Selenium-контекст на bg для фетчу), `Target.activateTarget(main)` ПІСЛЯ (UI браузера завжди на головній вкладці); `main_handle` винесений на початок методу і доступний у fallback-гілці `new_window` (там теж activateTarget(main) після створення вкладки). Фетчі працюють тихо, жодних стрибків вкладок.
3. **`_tile_hint`:** `empty` додано до чистих статусів (`None, "ok", "idle", "hidden", "empty"`) → плитка Reddit без хінту при 0 постах, узгоджено з «✓ Прив'язано» на вкладці.

Верифікація: AST OK; smoke 4 сценарії (порядок плиток == вкладок; `_tile_hint("empty") == ""`, blocked/no_key/error без змін; `_open_bg_tab`: switch → activateTarget(main) у правильному порядку; fallback: new_window → activateTarget(main) → about:blank). build v1.0.30 (8.6 MB, _internal 69) через TMP-обхід (canonical dir знову заблокований невидимим хендлом); адмінка запущена (14:45). Юзер має бачити: плитки в тому ж порядку, що вкладки; при вході в Community — жодних нових вкладок/стрибків сторінок; плитка Reddit зелена «Прив'язано» без помилки.

---

## Адмінка v1.0.29 (21.08.2026): плитка Overview + жорстка прив'язка плитка→сторінка браузера

Юзер: «Треба додати плитку оверв'ю. Перевірити щоб кожній плитці відповідала своя сторінка у браузері — не треба відкривати інші закладки і треба до кожної плитки прив'язати свою сторінку у браузері».

**Корінь «плитка ≠ сторінка»:** клік по плитці з головного вікна → `_enter_community()` завжди спочатку навігував Огляд (`_select_tab("overview")` + `_nav_overview` через 1200мс), і лише через 250мс `_select_tab(tab)` перемикав на вкладку плитки → `_navigate_platform_tab(tab)`. Race: `_nav_overview` перезаписував навігацію назад на admin.html (браузер лишався на чужій сторінці); поки драйвер `creating` — навігація тихо губилась (`drv is None` → return); клік по плитці ВЖЕ активної вкладки не давав `<<NotebookTabChanged>>` → навігації немає взагалі; фетч-вкладка у фоновому CDP могла перехопити `drv.get`.

**Фікс:**
1. **Плитка Overview** — перша в стріпі (`_build_tile_strip`, спільний для головного вікна і fullscreen): значення = `builds_ver` з RTDB (стан конвеєра AI-білдів), хінт порожній; i18n `tile_overview` EN "Overview" / UK "Огляд" + `h_comm_tiles_d` оновлено (197/197 ключів seed, без дифів).
2. **Пряма прив'язка плитка→сторінка:**
   - `_enter_community(target_tab=None)` — вхід з плитки: одразу `_select_tab(target_tab)` і `_nav_to(target_tab)`-ретрай (без Огляд-флешу); вхід через кнопку Community — Огляд як раніше (v1.0.28 поведінка збережена, `_nav_overview` → `_nav_to`).
   - `_open_community_tab` fs-гілка: після `_select_tab(tab)` — **явна** `_navigate_platform_tab(tab)` (покриває клік на вже активній вкладці).
   - `_navigate_platform_tab._go`: `switch_to.window(handles[0])` перед `drv.get(url)` — навігація тільки в головній вкладці юзера, фетч-вкладка не перехоплює, нових закладок не відкривається.

Верифікація: AST OK; seed round-trip 197/197; smoke: tile strip (порядок overview→youtube→github→kofi→reddit→installs→errors, значення 17/«—»), 7 навігаційних сценаріїв (вхід з плитки → target одразу; fs-клік на активній вкладці → явна навігація; навігація тільки в main-вкладці; errors/apikeys не навігують; youtube без channel_id → youtube.com; вхід з кнопки → Огляд). build v1.0.29 (8.6 MB, _internal 69) через TMP-обхід; адмінка запущена. Юзер має бачити: плитку Overview першою з версією білдів; клік на будь-яку плитку → браузер одразу на сторінці цієї платформи (жодних чужих сторінок/вкладок).
