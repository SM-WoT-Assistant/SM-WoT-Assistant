...
# We'll locate from "    def _finish_filter_with_items(self, items_to_show):" to next "    def " at same indent level
lines = content.split('\n')
start = None
end = None
for i, line in enumerate(lines):
    if line.strip().startswith('def _finish_filter_with_items('):
        start = i
    elif start is not None and i > start:
        # Check if line starts with '    def ' (4 spaces + def) and is not continuation
        if line.startswith('    def ') and not line.strip().startswith('def _finish_filter_with_items('):
            end = i
            break
        # Also check for class definition
        if line.startswith('class ') or (line.startswith('#') and i > start + 5):
            # Probably not, but break if we hit a class
            pass
...
if end is None:
    # Maybe next method not found, go to end of file
    end = len(lines)

print(f"Replacing lines {start} to {end}")

# New method text (with correct indentation)
