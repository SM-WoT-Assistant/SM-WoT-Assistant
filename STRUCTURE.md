...
├── VERSION                     # Plaintext semver (1.0.67) — consumed by config.load_version() and build.py
├── admin_version.txt           # Plaintext semver for Admin Desktop App (1.0.6) — independent of main VERSION; read by admin_app.py and build_admin.py, bundled via --add-data
...
├── build.py                    # PyInstaller build orchestrator + GitHub release (always creates release, gh CLI + NSIS required); dirty git working tree blocks build; generate_verify_json() writes public/verify.json + popular_tanks_seed.json; 4-phase release audit (verify → create → audit → write → audit); verify_launcher_bundle() checks launcher for Tcl/Tk DLLs + _imaging; write_version_to_rtdb() publishes versions; all builds Beta/prerelease by default
...
├── admin_app.py                # Desktop admin tool (tkinter GUI): build generation, scripts.pkg scanning, WG API game version monitoring, auto-detect changed tanks, generate builds via Gemini, tray icon, settings persistence, --tray CLI; version from admin_version.txt; single-instance mutex SM_WoT_Assistant_Admin_SingleInstance; i18n EN/UK (_TR_EN dict, admin_uk_cache.json + admin_uk_seed.json); _log() appends to admin.log; _resolve_wot_path() auto-detect
├── admin_build_generator.py    # Admin daemon: --popular, --builds, --listen modes; polls pending_updates/builds every 10s, generates builds via Gemini using generate_prompt_v2 + _fill_all_builds.parse_build, publishes to RTDB builds/tanks/{tag}; manifest fingerprinting (detect_changed_tanks, snapshot_manifest, update_manifest_for_tags); _MANIFEST_LOCK guards detect/snapshot/update
...
├── tank_extractor.py           # Tank data extraction from game client; update_compact_descr() populates missing compact_descr fields; uses name_localizer + tth_updater
...
├── database.rules.json         # RTDB security rules + indexes (versions, schemes, error_reports, installations, service_events, groups, user_groups)
...
- Contains: 45 Python scripts, 27 JSON data files, TTF fonts, `.mo` localization files, markdown docs, SVG icons, `installer.nsi`
...
**New Firebase feature:** Add to `firebase_reporter.py` for analytics/reporting, `firebase_drawings.py` for cloud data sync, or create new `firebase_*.py` module following the REST API pattern (API key auth, `_rtdb_url()` path builder, `_put`/`_post` helpers; use `requests.put(data=b"null")` for deletions)
