...
```
[project-root]/
├── main.py                     # Application entry point, WotAssistantHQ hub class, single-instance mutex, root.attributes("-topmost", True) at startup, active_view="" (no view loaded initially), splash update check with animated dots, CLI --ai-webview routing, CLI --splash-geometry= passthrough from launcher, CLI --tray for minimized startup (skips splash → _start_startup_checks() → _minimize_to_tray); _start_to_tray flag guards splash wait and auto-update check; _launch_on_game_start_var / _start_minimized_var / _close_with_game_var persisted to settings; _set_windows_startup(enable) manages HKCU\Software\Microsoft\Windows\CurrentVersion\Run entry preferring tray_watcher.exe over launcher.exe --tray fallback via winreg; _get_launcher_exe() finds launcher path in install dir or bundle dir; _get_tray_watcher_exe() finds tray_watcher.exe in install dir or bundle dir; F10 minimize-to-system-tray (creates/removes TrayIcon, shows _show_restore_button floating Toplevel, hides with _hide_restore_button), F8 Lock/Unlock button (format_mode_enabled — enables hotkeys without Ctrl), E→TAB hotkey change, no-Ctrl arrow/shift hotkeys for opacity/contrast, _hidden_by_f10 guard flag blocks overlay paths, _startup_flush_service / _periodic_flush_service_messages (300s), _on_root_show startup guard, ComboLBox overlay fixes, save/restore edit_h geometry, window position clamped to screen bounds on initialize_window(); group state fields (active_group_id, _cached_groups, _group_id_map, _sync_running); group sync polling methods (_start_group_sync, _stop_group_sync, _sync_schedule, _sync_cycle, _show_group_sync_notification via overrideredirect custom dialog); group cache methods (_load_group_schemes_from_cache, _save_group_schemes_to_cache); ask_clear_confirm uses dialog_utils.make_custom_dialog + _DragHelper; process_queue() packs second_row for registered users with more than one group
├── launcher.py                  # Standalone pre-startup update checker + background tray watcher (built as --onefile PyInstaller binary, imports unicodedata); auto-detects installed version via _detect_installed_version() (regex handles "Beta" suffix), _clean_old_versions() removes stale EXEs at startup, supports --skip-splash for silent relaunch after update, shows splash with animated dots, checks RTDB for updates synchronously, downloads/installs, then launches main EXE (or launcher --skip-splash after update); supports --tray watcher mode: lightweight background process (no window) with WinAPI tray icon (Shell_NotifyIconW via NOTIFYICONDATAW/WNDCLASSW structures), polls every 1s via _tray_poll() — left-click launches main app (respects start_minimized), right-click shows tk.Menu ("Launch"/"Exit"), monitors WorldOfTanks.exe via CreateToolhelp32Snapshot for auto-launch on game start (gated by launch_on_game_start setting); uses separate SM_WoT_Assistant_Watcher mutex; reads settings from %APPDATA%/SM WoT Assistant/settings.json; _launch_main() reads start_minimized and passes --tray to main.exe
├── tray_watcher.py              # Minimal background process monitor (pure stdlib + ctypes, no tkinter/PIL/requests); built as --onefile PyInstaller binary. Watches WorldOfTanks.exe via CreateToolhelp32Snapshot at 5s intervals. Launches SM WoT Assistant Launcher.exe on game start; closes SM WoT Assistant v*.exe on game stop when close_with_game setting enabled. Uses no hidden imports
...
├── config.py                   # Paths (BASE_DIR, ICON_FILE for icon.ico, LOGO_FILE), constants, load_version() (reads VERSION and appends " Beta" suffix), TECH_MAPS_STAGING mapping, MAP_NAMES_EN English map name dictionary, GROUP_CACHE_FILE path for group_schemes_cache.json
├── VERSION                     # Plaintext semver (e.g., 1.0.0) — consumed by config.load_version() and build.py
├── build.py                    # PyInstaller build orchestrator + GitHub release (always creates release, --release flag removed, gh CLI required, NSIS required); supports --beta flag (adds "Beta" to GitHub title, RTDB display_version, and summary — filenames stay clean semver); supports --date=YYYY-MM-DD for RTDB release_date override; RTDB publish only after successful GitHub release; writes display_version field to RTDB versions nodes; build_tray_watcher() builds tray_watcher.py as --onefile into onedir output
├── wot_assistant.spec          # PyInstaller spec: hidden imports, COLLECT step for onedir output; runtime_jsons contains only VERSION
├── installer.nsi                # NSIS installer script (lzma-compressed, InstallDir $LOCALAPPDATA, RequestExecutionLevel user — no admin); deletes old SM WoT Assistant v*.exe before install, cleans old v*.lnk shortcuts via wildcard, auto-launches new version post-install via Exec "$INSTDIR\SM WoT Assistant v${PRODUCT_VERSION}.exe"; shortcuts are version-agnostic (SM WoT Assistant.lnk, not vX.Y.Z.lnk)
...
├── ui_manager.py               # UI layout, view switching, filter construction; top_bar with Lock/Unlock button (btn_format_lock, FontAwesome lock/unlock icons), minimize-to-tray button (FontAwesome chr(0xF068)), settings gear (FontAwesome chr(0xF013)), Website link in settings menu; battle mode labels from game client .mo via language_module.get_lang_module() with Google fallback; group selector (ttk.Combobox, shown only for registered users with groups), group token button (invite code copy for officer / 🔒 for member), second_row for group UI under toolbar; _on_group_select changes active_group_id, starts/stops sync, reloads map list filtered by group schemes, refreshes linked schemes list in palette; _build_identity_bar() creates identity bar (nickname + PIN display + edit + action button) packed below top_bar; _refresh_identity_bar() shows Register/Connect/Disconnect state; _show_registration_dialog() with nickname availability check (debounced 400ms), PIN entry masked, LOG IN button; second_row packing/unpacking respects registration + active_view state; settings menu includes close_with_game checkbox (tray_watcher terminates main app on WoT exit)
...
├── help_system.py              # F1 help overlay via Toplevel grid of Label widgets (table layout); auto-sized window via winfo_reqwidth/reqheight (no hardcoded size); sections for draw, palette, Export/Import, filters, Group Schemes; F8 section uses FontAwesome lock icons (chr(0xF023)/chr(0xF09C)); editor and battle Ctrl+ sections consolidated/removed; E→TAB, F8→Lock/Unlock; Group Schemes section describes group selector, token button, Create/Join/Manage dialogs, publish-to-group
...
├── translations.py             # Default UI/map translations (English-only authoritative source); class labels (LT/MT/HT/TD/SPG) kept in English for all languages; close_with_game key added for tray watcher setting
...
├── firebase_reporter.py        # RTDB REST: error reports, version pings, update checks, service events; compare_versions() strips " Beta" suffix before semver comparison
...
│   ├── index.html              # English-only landing page (logo 2x, download button linking to GitHub releases, features grid, community schemes promo; no releases table — latest version label read dynamically from RTDB versions/ node; download button text uses RTDB display_version field)
...
**Entry Points:** `main.py`: Application startup, single-instance mutex (`CreateMutexW`), creates `WotAssistantHQ` with all managers, `--ai-webview` CLI routing, `--splash-geometry=` CLI flag passthrough from launcher, `--tray` CLI flag for minimized startup (skips splash, runs checks directly, minimizes to tray), `_start_to_tray` guard, `os.chdir(config.BASE_DIR)`, AppData seeding via `config.DEFAULT_FILES`, Firebase global excepthook + ping + splash-synchronous update check (`check_for_updates_sync`) with animated dots, F10 minimize-to-system-tray (creates/removes TrayIcon, `_hidden_by_f10` guard), F8 Lock/Unlock via `btn_format_lock` (format_mode_enabled enables hotkeys without Ctrl), hotkey `e`→`tab`, no-Ctrl arrow/shift hotkeys for opacity/contrast, save/restore `edit_h` geometry, `_set_windows_startup()` manages `HKCU Run` entry preferring `tray_watcher.exe` over `launcher.exe --tray` fallback, `_get_launcher_exe()` finds launcher path, `_get_tray_watcher_exe()` finds tray_watcher path, `launch_on_game_start`/`start_minimized`/`close_with_game` settings persisted; `launcher.py`: Standalone pre-startup launcher + background tray watcher (built as `--onefile`), auto-detects installed version via `_detect_installed_version()`, shows own splash with animated dots (`_animate_dots` rescheduled unconditionally), checks RTDB for updates, downloads/installs (NSIS `Exec` auto-launches post-install), launches `--skip-splash` for clean restart after update; `--tray` mode: lightweight tray icon (WinAPI `Shell_NotifyIconW`) polling every 1s, WoT process detection via `CreateToolhelp32Snapshot`, auto-launch on game start; `_launch_main()` reads `start_minimized` from settings and passes `--tray` flag to main.exe; `build.py`: 7-phase PyInstaller onedir build + NSIS installer + portable ZIP + GitHub release + RTDB version publish (supports `--date=YYYY-MM-DD` for release_date override); `build_tray_watcher()` builds tray_watcher.py as --onefile into onedir
**Configuration:** `config.py`: File paths (`BUNDLE_DIR` for bundled assets, `USER_DATA_DIR` for writable AppData at `%APPDATA%/SM WoT Assistant`, `ICON_FILE` for `icon.ico`), `load_version()`, `TECH_MAPS_STAGING` map name mappings, `MAP_NAMES_EN` English map name dictionary, `DEFAULT_FILES` seeding list, `GROUP_CACHE_FILE` (group_schemes_cache.json path); `VERSION`: Plaintext semver (currently `1.0.52`) consumed by config, build, and launcher; `wot_assistant.spec`: PyInstaller packaging spec (COLLECT step for onedir output, produces `SM WoT Assistant.exe` → renamed to `SM WoT Assistant vX.Y.Z.exe` by `build.py`); `build.py:_CRITICAL_JSON`: 6 verified JSONs (`crew_builds.json`, `tank_db.json`, `tank_tth.json`, `game_entities_english.json`, `game_entities.json`, `tank_slots_full.json`); `installer.nsi`: NSIS installer script (`InstallDir "$LOCALAPPDATA\SM WoT Assistant"`, `RequestExecutionLevel user`, lzma, standard pages); deletes old `SM WoT Assistant v*.exe` before install, cleans shortcuts via wildcard, auto-launches new version post-install via `Exec`; shortcuts are version-agnostic (`SM WoT Assistant.lnk`); `settings.json`: Runtime user settings
**Core Logic:** `main.py` (class WotAssistantHQ): Central hub with settings for `launch_on_game_start`, `start_minimized`, `close_with_game`; `_set_windows_startup()` manages HKCU Run preferring tray_watcher; `_get_tray_watcher_exe()` locates tray_watcher.exe; `map_manager.py`: Map data and game version management, unified TACTIC↔MAPS list; `stats_ai.py` (class StatsAI): AI build system, subprocess webview launch; `tray_icon.py` (class TrayIcon): System tray icon via pure WinAPI ctypes (Shell_NotifyIconW), F10 minimize-to-tray flow creates/removes TrayIcon instance
**Window Management:** `window_manager.py`: Window positioning, resizing, transparency, click-through, game focus, drag in any mode when format_mode_enabled (F8/Lock); reads/writes edit_h from settings; `ui_manager.py`: UI layout construction, view switching, top_bar with Lock/Unlock button (FontAwesome lock icons), minimize-to-tray button (─), Website link in settings menu, battle mode filter labels from game client .mo (Google fallback), identity bar (nickname + PIN + edit + action), registration dialog with nickname availability check (debounced 400ms) and LOG IN button
