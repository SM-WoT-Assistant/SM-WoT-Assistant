...
├── admin_version.txt           # Plaintext semver for Admin Desktop App (e.g., 1.0.2) — independent of main VERSION; read by admin_app.py:_read_admin_version() and build_admin.py:read_version(), bundled via --add-data
...
├── parse_tiers_devices.py      # Equipment tier/device parser from tiers_devices.xml; used by generate_prompt_v2.py for dynamic equipment lists with hardcoded fallback
...
│   ├── admin.html              # Admin dashboard (stats, schemes management, version management, Admin App section shows static note — desktop admin EXE distributed separately as local build only via python admin_app.py, user management with PIN reset, error reports with desktop notifications, pending builds status polling)
