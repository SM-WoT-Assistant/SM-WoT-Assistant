...
├── launcher.py                  # Standalone pre-startup update checker (built as --onefile PyInstaller binary); auto-detects installed version via _detect_installed_version(), supports --skip-splash for silent relaunch after update, shows splash with animated dots, checks RTDB for updates synchronously, downloads/installs, then launches main EXE (or launcher --skip-splash after update)
...
├── painting_palette.py         # Floating DrawingPalette: toolbar, live-sync editing, 5-color history (_custom_colors), status bar, delete button, deselection; uses transient() + lift(aboveThis=) for stacking (no -topmost); battle mode checkboxes with .mo names via language_module.get_lang_module() (Google fallback), class checkboxes always English (LT/MT/HT/TD/SPG)
