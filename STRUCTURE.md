...
├── config.py                   # Paths (BASE_DIR), constants, load_version() (reads VERSION and appends " Beta" suffix), TECH_MAPS_STAGING mapping, MAP_NAMES_EN English map name dictionary, GROUP_CACHE_FILE path for group_schemes_cache.json
├── VERSION                     # Plaintext semver (e.g., 1.0.0) — consumed by config.load_version() and build.py
├── build.py                    # PyInstaller build orchestrator + GitHub release (always creates release, --release flag removed, gh CLI required, NSIS required); supports --beta flag (adds "Beta" to GitHub title, RTDB display_version, and summary — filenames stay clean semver); supports --date=YYYY-MM-DD for RTDB release_date override; auto-commits VERSION + installer.nsi via commit_version_files() before release; RTDB publish only after successful GitHub release; writes display_version field to RTDB versions nodes
