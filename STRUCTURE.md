...
├── config.py                   # Paths (BASE_DIR), constants, load_version(), TECH_MAPS_STAGING mapping, MAP_NAMES_EN English map name dictionary, GROUP_CACHE_FILE path for group_schemes_cache.json
...
├── ui_manager.py               # UI layout, view switching, filter construction; top_bar with Lock/Unlock button (btn_format_lock, FontAwesome lock/unlock icons), minimize-to-tray button (FontAwesome chr(0xF068)), settings gear (FontAwesome chr(0xF013)), Website link in settings menu; battle mode labels from game client .mo via language_module.get_lang_module() with Google fallback; group selector (ttk.Combobox, shown only for registered users with groups), group token button (invite code copy for officer / 🔒 for member), second_row for group UI under toolbar; _on_group_select changes active_group_id, starts/stops sync, reloads map list filtered by group schemes
├── data_manager.py             # JSON file I/O, tank DB loading with fallback chain
├── dialog_utils.py             # Dark-themed dialog utilities: dark_messagebox(), dark_confirmbox(), dark_promptbox(), _set_dark_title_bar(), _center_on_root(), _DragHelper class for overrideredirect window dragging
...
├── firebase_groups.py           # Group system: create/join/leave/manage groups, group scheme publish/sync/import, invite codes, officer/member roles, combined public+group scheme fetch, in-memory _group_schemes_cache, RTDB groups/{gid}/schemes node
...
**Firebase/Cloud:** `firebase_identity.py`: Local identity + PIN auth with RTDB-backed `login()` function (`users/{user_id}` node), `register()` syncs to RTDB immediately; `firebase_reporter.py`: RTDB error/analytics/versions with `_pick_latest()` sorting by release_date → semver; `firebase_drawings.py`: RTDB drawing CRUD; `firebase_groups.py`: Group system — create/join/leave/manage groups with invite codes, officer/member roles, group scheme publish/sync/import; RTDB `groups/{gid}/schemes` and `user_groups/{uid}` nodes; `get_group_schemes()` with in-memory cache, `get_group_schemes_meta()` for lightweight sync polling, `get_combined_schemes()` aggregates public + all user's group schemes; `firebase.json` + `.firebaserc`: Firebase project config; `database.rules.json`: RTDB security rules with `groups/` indexes (invite_code, created_at); `public/`: English-only static website deployed to Firebase Hosting (download links point to `github.com/nkcgml-boop/SM-WoT-Assistant/releases`, releases sorted by release_date with version tiebreaker)
