...
├── build_admin.py               # Standalone build script for Admin Desktop App (admin_app.py); builds --onefile --windowed EXE via PyInstaller, local build only (no GitHub upload, no RTDB versions/admin publish), reads admin_version.txt; run separately via python build_admin.py
