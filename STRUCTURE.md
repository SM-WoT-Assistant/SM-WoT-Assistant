...
├── config.py                   # Paths (BASE_DIR, USER_DATA_DIR=%APPDATA%/SM WoT Assistant, ICON_FILE, LOGO_FILE), constants, load_version() (reads VERSION + " Alpha"), TECH_MAPS_STAGING, MAP_NAMES_EN (37_caucasus → "Mountain Pass"), DEFAULT_FILES seeding list, GROUP_CACHE_FILE; verification constants (VERIFY_URL, VERIFY_CACHE_FILE, VERIFY_CACHE_DAYS, VERIFIED_MARKER, INSTALL_REG_KEY, CRITICAL_BUNDLE_FILES list of 9 critical files)
├── VERSION                     # Plaintext semver (1.0.70) — consumed by config.load_version() and build.py
├── admin_version.txt           # Plaintext semver for Admin Desktop App (1.0.16) — independent of main VERSION; read by admin_app.py and build_admin.py, bundled via --add-data
...
├── build.py                    # PyInstaller build orchestrator + GitHub release (always creates release, gh CLI + NSIS required); dirty git working tree blocks build; generate_verify_json() writes public/verify.json + popular_tanks_seed.json; 4-phase release audit (verify → create → audit → write → audit); verify_launcher_bundle() checks launcher for Tcl/Tk DLLs + _imaging; write_version_to_rtdb() publishes versions via admin_auth ID token (fails without admin_creds.json); all builds Alpha/prerelease by default
...
├── build_admin.py              # Standalone build script for Admin Desktop App; --onedir --windowed into dist/SM WoT Assistant Admin/ (canonical launch location); clean() removes folder + root onefiles; reads admin_version.txt; selenium 4 hidden imports + deep_translator; post-build _internal guard (verifies _internal exists, non-empty, python312.dll present — FATAL + exit(1) on broken COLLECT); local build only
├── admin_icon.ico              # Red border icon for admin app
├── installer.nsi               # NSIS installer script (lzma, InstallDir $LOCALAPPDATA, RequestExecutionLevel user — no admin); kills tray watcher via nsExec::ExecToStack 'taskkill /f /im "SM WoT Assistant Tray Watcher.exe"' (hidden launch, no terminal flash); deletes old SM WoT Assistant v*.exe; writes installed=1 registry key; auto-launches new version via Exec; version-agnostic shortcuts
...
├── log_reader.py               # python.log tailing for battle event detection; type-to-mode mapping (1→ctf, 2→domination, 3→assault, 4→comp7, 5→comp7_light); hangar_re matches client hangars + event spaces (h<N>_* like h42_Wot_Bday_2026), both excluded from arena detection
...
├── .github/                    # GitHub repo metadata (FUNDING.yml — ko_fi sponsor + Monobank custom link, repo Sponsor button)
