...
├── wot_assistant.spec          # PyInstaller spec file: explicit datas, hidden imports, ~115 MB bundle
...
├── map_manager.py              # Map list loading (unified TACTIC↔MAPS from extracted_maps/map_dictionary.json), game version check, data updates, TACTIC image existence filter
...
│       └── DLLs/
├── build/                      # PyInstaller build output (dist/, localpycs/, Analysis, PYZ, EXE)
│   └── wot_assistant/          # Bundled EXE directory
...
- Contains: 150+ Python scripts, JSON data files, TTF fonts, markdown docs, SVG icons
...
**tools/orion/:**
- Purpose: Embedded Orion decoder tool for XML decoding
- Contains: Orion executable and its DLLs

**build/:**
- Purpose: PyInstaller build output directory
- Contains: `wot_assistant/` (bundled EXE and dependencies), Analysis/PYZ/EXE toc files, base_library.zip
...
**Entry Points:** `main.py`: Application startup, creates `WotAssistantHQ` with all managers, `--ai-webview` CLI routing, `os.chdir(config.BASE_DIR)`; `build.py`: PyInstaller build + GitHub release
**Configuration:** `config.py`: File paths (BASE_DIR), load_version(), TECH_MAPS_STAGING map name mappings; `VERSION`: Plaintext semver consumed by config and build; `wot_assistant.spec`: PyInstaller packaging spec; `settings.json`: Runtime user settings
...
**New deployment target:** Update `wot_assistant.spec` datas/hiddenimports lists, or extend `build.py` with new packaging steps
