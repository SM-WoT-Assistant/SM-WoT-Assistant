...
```
[project-root]/
├── main.py                     # Application entry point, WotAssistantHQ hub class, single-instance mutex, splash update check with animated dots, CLI --ai-webview routing, CLI --splash-geometry= passthrough from launcher, F10 closes drawing palette, _startup_flush_service / _periodic_flush_service_messages (300s), _on_root_show startup guard, ComboLBox overlay fixes
...
├── painting_palette.py         # Floating DrawingPalette: toolbar, live-sync editing, 12-color history (_default_colors), status bar, delete button, deselection, Publish/Publish All/Save/Save All/Import/Download buttons; community schemes download dialog with map/author filters and preview; uses transient() + lift(aboveThis=) for stacking (no -topmost); battle mode checkboxes with .mo names via language_module.get_lang_module() (Google fallback), class checkboxes always English (LT/MT/HT/TD/SPG)
...
**Drawing System:** `painter.py` (class MapPainter): Tactical markers/arrows/text with Ctrl+Z undo, resize, right-click edit/delete with `dialog_utils.dark_confirmbox` confirmation, edit-mode selection rectangle, _draw_class_icons filtered against active checkboxes, overlay canvas (_po_win Toplevel) with transient/Unmap/Map bindings, _po_sync_timer 500ms, _on_root_show startup guard; `painting_palette.py` (class DrawingPalette): Floating non-modal palette with toolbar (marker, XVMSymbol class icons, tree FontAwesome icon, 8 tactical icons), mode checkboxes with names from game client .mo (Google fallback), class checkboxes (LT/MT/HT/TD/SPG — always English), text entry, 12-color picker (_default_colors array), delete button, status bar, deselection on click-away; two publish rows — Publish/Publish All with auto-translated descriptions and duplicate detection; two save/import rows — Save/Save All/Import (unified via `import_unified()`)/Download (community schemes browser with map/author filters and preview, Replace/Add/Save-to-PC choice); `tactics_manager.py`: `export_all_tactics()` / `import_all_tactics()` with Replace/Merge choice dialog, `import_unified()` auto-detects single-map vs all-maps format, all dialogs use `dialog_utils`
