...
├── build.py                    # PyInstaller build orchestrator + GitHub release (always creates release, --release flag removed, gh CLI required, NSIS required); supports --beta flag (adds "Beta" to GitHub title, display_version, summary, and _Beta suffix to installer/ZIP filenames; --prerelease flag for beta); supports --date=YYYY-MM-DD for RTDB release_date override; dirty git working tree blocks build (fatal error); 4-phase release audit: verify_release_artifacts() → create_github_release() → audit_github_release() (deletes release on mismatch) → write_version_to_rtdb() → audit_rtdb_entry(); verify_launcher_bundle() checks launcher onefile for Tcl/Tk DLLs; RTDB publish only after successful GitHub release + audit; writes display_version field to RTDB versions nodes; build_tray_watcher() builds tray_watcher.py as --onefile into onedir output
