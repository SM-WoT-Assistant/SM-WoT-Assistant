...
```
[project-root]/
├── main.py                     # Application entry point, WotAssistantHQ hub class, single-instance mutex, splash update check with animated dots, CLI --ai-webview routing, CLI --splash-geometry= passthrough from launcher, F10 closes drawing palette
├── launcher.py                  # Standalone pre-startup update checker (built as --onefile PyInstaller binary); shows splash, checks RTDB for updates synchronously, downloads/installs, then launches main app with --splash-geometry=
...
├── build.py                    # PyInstaller build orchestrator + GitHub release via gh CLI; supports --date=YYYY-MM-DD for RTDB release_date override
├── wot_assistant.spec          # PyInstaller spec: hidden imports, COLLECT step for onedir output; runtime_jsons contains only VERSION
├── installer.nsi                # NSIS installer script (lzma-compressed, InstallDir $LOCALAPPDATA, RequestExecutionLevel user — no admin)
...
├── help_system.py              # F1 help overlay via Toplevel grid of Label widgets (table layout); sections for editor, battle, draw, palette, Export/Import, filters
...
**Entry Points:** `main.py`: Application startup, single-instance mutex (`CreateMutexW`), creates `WotAssistantHQ` with all managers, `--ai-webview` CLI routing, `--splash-geometry=` CLI flag passthrough from launcher, `os.chdir(config.BASE_DIR)`, AppData seeding via `config.DEFAULT_FILES`, Firebase global excepthook + ping + splash-synchronous update check (`check_for_updates_sync`) with animated dots, F10 toggles visibility and closes drawing palette; `launcher.py`: Standalone pre-startup launcher (built as `--onefile`), shows own splash, checks RTDB for updates, downloads/installs, launches main app; `build.py`: 7-phase PyInstaller onedir build + NSIS installer + portable ZIP + GitHub release + RTDB version publish (supports `--date=YYYY-MM-DD` for release_date override)
**Configuration:** `config.py`: File paths (`BUNDLE_DIR` for bundled assets, `USER_DATA_DIR` for writable AppData at `%APPDATA%/SM WoT Assistant`), `load_version()`, `TECH_MAPS_STAGING` map name mappings, `DEFAULT_FILES` seeding list; `VERSION`: Plaintext semver (currently `1.0.16`) consumed by config, build, and launcher; `wot_assistant.spec`: PyInstaller packaging spec (COLLECT step for onedir output, produces `SM WoT Assistant.exe` → renamed to `SM WoT Assistant vX.Y.Z.exe` by `build.py`); `build.py:_CRITICAL_JSON`: 6 verified JSONs (`crew_builds.json`, `tank_db.json`, `tank_tth.json`, `game_entities_english.json`, `game_entities.json`, `tank_slots_full.json`); `installer.nsi`: NSIS installer script (`InstallDir "$LOCALAPPDATA\SM WoT Assistant"`, `RequestExecutionLevel user`, lzma, standard pages); `settings.json`: Runtime user settings
...
**Drawing System:** `painter.py` (class MapPainter): Tactical markers/arrows/text with Ctrl+Z undo, resize, right-click edit/delete, edit-mode selection rectangle, _draw_class_icons filtered against active checkboxes, overlay canvas (_po_win Toplevel) with transient/Unmap/Map bindings, _po_sync_timer 500ms; `painting_palette.py` (class DrawingPalette): Floating non-modal palette with two button rows — Save/Load (single tactic) and ALL EXPORT/ALL IMPORT (bulk operations via `tactics_manager.py`), toolbar (marker, XVMSymbol class icons, tree FontAwesome icon, 8 tactical icons), mode checkboxes with names from game client .mo (Google fallback), class checkboxes (LT/MT/HT/TD/SPG — always English), text entry, 5-color history color picker, delete button, status bar, deselection on click-away; `tactics_manager.py`: `export_all_tactics()` / `import_all_tactics()` with Replace/Merge choice dialog
